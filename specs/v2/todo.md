...
- remove the public in-memory `@bolt-ai/llm` tool loop after replacing its
...
- plugins get global "bolt" instance like in that post i showed
- opencode instance has stuff like `bolt.session.prompt()` or
  `bolt.tool.register({...})`
...
- decide whether to preserve deprecated `@bolt-ai/llm` orchestration exports
