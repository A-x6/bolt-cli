...
V2 core discovers config documents named `bolt.json` or `bolt.jsonc` in the global config directory, ancestor project directories, and `.bolt` config directories. The legacy `config.json` filename is not supported in V2.
...
The configured `plugins` list represents package-loaded plugins only. Local plugin code remains discovered from plugin directories such as `.bolt/plugins/`; v2 does not port arbitrary configured local paths or file URLs into this field.
...
Policy evaluation will consume authored config documents in reverse order while preserving statement order inside each document. The precedence of `.bolt` policy sources remains open until `.bolt` configuration is reviewed.
