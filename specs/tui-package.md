...
Move the canonical Bolt terminal application from
...
`packages/cli`, or `@bolt-ai/core`.

The SDK is the TUI's Bolt boundary. Missing backend data or operations must
...
- Do not replace private `packages/opencode` imports with `@bolt-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@bolt-ai/tui`.
...
- Bolt domain data displayed by the TUI
...
Status: Completed. The private `@bolt-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@bolt-ai/tui`.
...
- The package has no dependency on `opencode`, `@bolt-ai/cli`, or
  `@bolt-ai/core`.
...
primitives now live in `@bolt-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@bolt-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Bolt globals or process state;
...
Replace process-global Bolt state with resolved TUI inputs.
...
`@bolt-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@bolt-ai/tui`. Configured references resolve through the new
...
adapters rather than Bolt domain implementations.

Make the SDK the only Bolt domain boundary used by the TUI.
...
- Domain-facing TUI code imports Bolt data and operations only from
  `@bolt-ai/sdk`.
...
`@bolt-ai/tui`. The legacy host injects a narrow plugin host that retains
...
public `@bolt-ai/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@bolt-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@bolt-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Bolt domain state.
...
rg '@bolt-ai/core|packages/opencode|packages/cli' packages/tui
