...
const model = "bolt/deepseek-v4-flash"
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with bolt...`)
  ...
  try {
    await $`bolt run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  bolt failed: ${err}`)
    return false
  }
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with bolt...")
