...
import { createBolt } from "@bolt-ai/sdk"
...
`)
  ...
  const bolt = await createBolt({ port: 0 })
  ...
  try {
    ...
    const session = await bolt.client.session.create()
    const result = await bolt.client.session
      .prompt({
