...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "bolt/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".bolt/glossary/zh-cn.md"
  if (locale === "zht") return ".bolt/glossary/zh-tw.md"
  return `.bolt/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Bolt did not report a session ID.")
  return match[1]
}
...
export function sessionModels(value: unknown) {
  ...
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Bolt session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Bolt runs for 'all' (default: 4)
      --model <provider/id>  Bolt model (default: bolt/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Bolt
...
`)
  ...
  if (failed.length) console.error(`\nBolt failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.BOLT_DISABLE_PROJECT_CONFIG = "1"
  env.BOLT_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["bolt", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.BOLT_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["bolt", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.BOLT_CONFIG
  delete env.BOLT_CONFIG_DIR
  delete env.BOLT_CONFIG_CONTENT
  delete env.BOLT_PERMISSION
  delete env.BOLT_AUTO_SHARE
  return env
}
