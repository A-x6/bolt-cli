...
describe("translate app", () => {
  ...
  test("maps product locale codes to their glossaries", () => {
    expect(glossaryFile("fr")).toBe(".bolt/glossary/fr.md")
    expect(glossaryFile("zh")).toBe(".bolt/glossary/zh-cn.md")
    expect(glossaryFile("zht")).toBe(".bolt/glossary/zh-tw.md")
  })
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "bolt/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
...
{"variants":{}}
`
    expect(modelVariants(output, "bolt/gpt-5.5")).toEqual({
  ...
  test("disables side effects and scopes edits for the translation agent", () => {
    const config = translationConfig("translate-app-fr", "bolt/gpt-5.5", ["packages/app/src/i18n/fr.ts"])
