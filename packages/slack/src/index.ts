...
import { createBolt, type ToolPart } from "@bolt-ai/sdk"
...
console.log("🚀 Starting bolt server...")
const bolt = await createBolt({
  port: 0,
})
console.log("✅ Bolt server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await bolt.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new bolt session...")
    const { client, server } = bolt
    ...
    console.log("✅ Created bolt session:", createResult.data.id)
  ...
  console.log("📝 Sending to bolt:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Bolt response:", JSON.stringify(result, null, 2))
