...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("bolt-find")
  api.delete("bolt-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("bolt-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("bolt-find", new Highlight(...rest))
    return true
  }
