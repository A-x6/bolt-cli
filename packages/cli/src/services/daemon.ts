...
import { createBoltClient } from "@bolt-ai/sdk/v2/client"
import { ServerAuth } from "@bolt-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createBoltClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@bolt/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createBoltClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createBoltClient({ baseUrl: connection.url, headers: connection.headers })
    })
