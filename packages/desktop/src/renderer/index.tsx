...
import type { UpdaterState } from "@bolt-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@bolt-ai/ui/logo"
import { useTheme } from "@bolt-ai/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.BOLT_CHANNEL === "prod" &&
...
const deepLinkEvent = "bolt:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__BOLT__ ??= {}
  const pending = window.__BOLT__.deepLinks ?? []
  window.__BOLT__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://bolt.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("bolt.global.dat").getItem("language")
