...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.bolt.desktop.dev",
  beta: "ai.bolt.desktop.beta",
  prod: "ai.bolt.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.bolt.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// The legacy `bolt.settings.dat` is special: it maps to the settings store
...
// (e.g. `"default.dat"`, `"bolt.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // The legacy bolt.settings.dat → the electron settings store (SETTINGS_STORE).
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "bolt.global.dat").
  const storeName = filename === "bolt.settings.dat" ? SETTINGS_STORE : filename
