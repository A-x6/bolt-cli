...
export function createWslServersController(
  ...
  const setBoltCheck = (distro: string, check: WslBoltCheck) => {
    setState({
      boltChecks: {
        ...state.boltChecks,
        [distro]: check,
  ...
  const checkBolt = async (distro: string, opts?: { signal?: AbortSignal }) => {
    ...
    const version = resolved
      ...
      : null
    return boltCheck(distro, resolved, version, appVersion)
  }

  const refreshBoltCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setBoltCheck(distro, await checkBolt(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const boltChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.boltChecks[distro])
        .map(async (distro) => [distro, await checkBolt(distro, opts)] as const),
    )
    if (boltChecks.length) {
      setState({ boltChecks: { ...state.boltChecks, ...Object.fromEntries(boltChecks) } })
    }
  }
  ...
  const refreshBoltCheckBackground = (id: string, distro: string) => {
    void checkBolt(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setBoltCheck(distro, check)
      })
      .catch((error) => {
  ...
  const refreshBoltChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkBolt(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setBoltCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshBoltCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installOpencode(name: string) {
      await runJob({ kind: "install-opencode", distro: name, startedAt: Date.now() }, async (abort) => {
        ...
        await refreshBoltCheck(name, { signal: abort.signal })
        expectOpencodeVersion(state.boltChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.boltChecks, distro) : {}),
      })
    },
