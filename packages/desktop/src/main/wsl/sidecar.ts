...
export async function spawnWslSidecar(
  ...
  const bolt = await resolveWslOpencode(distro)
  if (!bolt) throw new Error(`Bolt is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export BOLT_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export BOLT_CLIENT=desktop",
    `export BOLT_SERVER_USERNAME=${shellEscape(username)}`,
    `export BOLT_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(bolt)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
