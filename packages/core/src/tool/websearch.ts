...
export class ConfigService extends Context.Service<ConfigService, Config>()("@bolt/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.BOLT_WEBSEARCH_PROVIDER === "exa" || process.env.BOLT_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.BOLT_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("BOLT_EXPERIMENTAL") || truthy("BOLT_ENABLE_EXA") || truthy("BOLT_EXPERIMENTAL_EXA"),
    enableParallel: truthy("BOLT_ENABLE_PARALLEL") || truthy("BOLT_EXPERIMENTAL_PARALLEL"),
