...
describe("authorization redirect validation", () => {
  test("allows registered Bolt callbacks", () => {
    expect(isAllowedAuthorizationRedirect("app", "https://bolt.ai/auth/callback")).toBe(true)
    expect(isAllowedAuthorizationRedirect("app", "https://dev.bolt.ai/auth/callback")).toBe(true)
  ...
  test("rejects unregistered clients and external redirects", () => {
    expect(isAllowedAuthorizationRedirect("other", "https://bolt.ai/auth/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://evil.example/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://bolt.ai.evil.example/callback")).toBe(false)
