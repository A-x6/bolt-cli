...
const original = {
  BOLT_SERVER_PASSWORD: Flag.BOLT_SERVER_PASSWORD,
  BOLT_SERVER_USERNAME: Flag.BOLT_SERVER_USERNAME,
  envPassword: process.env.BOLT_SERVER_PASSWORD,
  envUsername: process.env.BOLT_SERVER_USERNAME,
}
const auth = { username: "bolt", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.BOLT_SERVER_PASSWORD = original.BOLT_SERVER_PASSWORD
  Flag.BOLT_SERVER_USERNAME = original.BOLT_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.BOLT_SERVER_PASSWORD
  else process.env.BOLT_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.BOLT_SERVER_USERNAME
  else process.env.BOLT_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.BOLT_SERVER_PASSWORD = auth.password
  Flag.BOLT_SERVER_USERNAME = auth.username
  process.env.BOLT_SERVER_PASSWORD = auth.password
  process.env.BOLT_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.BOLT_SERVER_PASSWORD = undefined
  Flag.BOLT_SERVER_USERNAME = auth.username
  delete process.env.BOLT_SERVER_PASSWORD
  process.env.BOLT_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.BOLT_DISABLE_DEFAULT_PLUGINS
    process.env.BOLT_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.BOLT_DISABLE_DEFAULT_PLUGINS
      else process.env.BOLT_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
