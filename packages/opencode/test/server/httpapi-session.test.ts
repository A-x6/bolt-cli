...
const originalWorkspaces = Flag.BOLT_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.BOLT_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.BOLT_EXPERIMENTAL_WORKSPACES = true
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "bolt", "src")
