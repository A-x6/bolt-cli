...
import { EventSequenceTable } from "@bolt-ai/core/event/sql"
...
import { WorkspaceTable } from "@bolt-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  BOLT_AUTH_CONTENT: process.env.BOLT_AUTH_CONTENT,
  BOLT_EXPERIMENTAL_WORKSPACES: process.env.BOLT_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.BOLT_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@bolt.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.BOLT_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.BOLT_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.BOLT_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.BOLT_EXPERIMENTAL_WORKSPACES).toBe("true")
