...
it.instance("plan agent denies edits except .bolt/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".bolt/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".bolt", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.BOLT_TEST_HOME
      process.env.BOLT_TEST_HOME = test.directory
