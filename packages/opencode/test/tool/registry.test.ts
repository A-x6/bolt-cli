...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".bolt")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .bolt/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const bolt = path.join(test.directory, ".bolt")
      const tool = path.join(bolt, "tool")
  ...
  it.instance("ignores non-tool exports in .bolt/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".bolt", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools from .bolt/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const bolt = path.join(test.directory, ".bolt")
      const tools = path.join(bolt, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".bolt", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const bolt = path.join(test.directory, ".bolt")
        const customTools = path.join(bolt, "tools")
        const plugin = path.join(bolt, "node_modules", "@bolt-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(bolt, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@bolt-ai/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@bolt-ai/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".bolt", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(bolt, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@bolt-ai/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(bolt, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(bolt, "node_modules", "cowsay")
