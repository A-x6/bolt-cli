...
describe("bolt read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // BOLT_CONFIG_CONTENT={} we provide, no servers should be configured
  ...
  // `session list` reads the session DB. Fresh BOLT_TEST_HOME means
  ...
  // in production it'd be a path under BOLT_TEST_HOME / XDG_DATA_HOME.
