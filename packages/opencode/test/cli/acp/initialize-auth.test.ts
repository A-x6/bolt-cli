...
describe("bolt acp initialize/auth subprocess", () => {
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.agentInfo?.name).toBe("Bolt")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.authMethods?.[0]?.id).toBe("bolt-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "bolt-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.BOLT_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("bolt-login")
