...
import type { QuestionRequest } from "@bolt-ai/sdk/v2"
import { OpencodeKeymapProvider, registerOpencodeKeymap } from "@bolt-ai/tui/keymap"
import {
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "bolt", modelID: "gpt-5" },
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "bolt", modelID: "gpt-5" })
