...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "bolt.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "bolt.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "bolt.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "bolt.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["bolt.json", "bolt.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "bolt.json"),
        path.join(project, "bolt.jsonc"),
      ])
    })
  })
