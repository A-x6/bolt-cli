...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.BOLT_TEST ?? "")'), {
      env: {
        BOLT_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "BOLT_TEST_SHELL"], {
      shell: true,
      env: {
        BOLT_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("BOLT_TEST_SHELL=ok")
  })
