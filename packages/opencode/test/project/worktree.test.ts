    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(info.branch).toBe("bolt/my-feature")
        }),
      { git: true },
    )
    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const svc = yield* Worktree.Service
          yield* git(test.directory, ["branch", "bolt/my-feature"])
    ...
    it.instance(
      ...
      () =>
        withCreatedWorktree(undefined, ({ info }) =>
          Effect.gen(function* () {
            expect(info.name).toBeDefined()
            expect(info.branch ?? "").toStartWith("bolt/")
      ...
      () =>
        withCreatedWorktree({ name: "test-workspace" }, ({ info }) =>
          Effect.gen(function* () {
            ...
            expect(info.branch).toBe("bolt/test-workspace")
          }),
        ),
      { git: true },
    )
