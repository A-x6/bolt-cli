...
import { NonNegativeInt, PositiveInt } from "@bolt-ai/core/schema"
import { ConfigAttachmentV1 } from "@bolt-ai/core/v1/config/attachment"
import { ConfigLSPV1 } from "@bolt-ai/core/v1/config/lsp"
import { InvalidError } from "@bolt-ai/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by Bolt V1. Use V1 "permission" rules or run bolt2.',
      })),
    })
