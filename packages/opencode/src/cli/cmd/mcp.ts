...
import { ConfigMCPV1 } from "@bolt-ai/core/v1/config/mcp"
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in bolt.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .bolt/ subdirectory too)
  const candidates = [path.join(baseDir, "bolt.json"), path.join(baseDir, "bolt.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".bolt", "bolt.json"), path.join(baseDir, ".bolt", "bolt.jsonc"))
  }
  ...
  // Default to bolt.json if none exist
  return candidates[0]
}
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "bolt-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
