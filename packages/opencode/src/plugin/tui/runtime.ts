...
import { errorData, errorMessage } from "@bolt-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@bolt-ai/tui/context/theme"
import { Global } from "@bolt-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@bolt-ai/core/util/flock"
import { Flag } from "@bolt-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@bolt-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@bolt-ai/core/v1/config/plugin"
import { createCommandShim } from "@bolt-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@bolt-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".bolt"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".bolt", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".bolt", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.BOLT_PURE ? [] : pluginOrigins
    if (Flag.BOLT_PURE && pluginOrigins.length) {
    }
