...
import type { LLMClientShape } from "@bolt-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("bolt"))
    return { type: "unsupported", reason: "provider is not openai, bolt, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @bolt-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      ...
      // the @bolt-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
