...
import { optional } from "@bolt-ai/core/schema"
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@bolt-ai/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
  ...
  return {
    ...
    opencode: Effect.fnUntraced(function* (input: Info) {
      ...
      const ok =
        ...
        Boolean((yield* dep.config()).provider?.["bolt"]?.options?.apiKey)
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>, model?: Model) {
          ...
          // 1. options.region from bolt.json provider config
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "bolt",
          },
        },
      }),
    kilo: () =>
...
export class Service extends Context.Service<Service, Interface>()("@bolt/Provider") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderV2.ID) {
      ...
      const priority = providerID.startsWith("bolt")
        ? ["gpt-nano"]
