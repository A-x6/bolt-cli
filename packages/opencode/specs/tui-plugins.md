...
- Author package entrypoint is `@bolt-ai/plugin/tui`.
...
- Import types from `@bolt-ai/plugin/tui`.
- `@bolt-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@bolt-ai/plugin/tui"
...
- The value is a semver range checked against the running Bolt version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.bolt` is absent, no check is performed (backward compatible).
...
- For local scope, path is `<worktree>/.bolt` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.bolt`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.bolt`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.bolt` dir or `BOLT_CONFIG_DIR`, Bolt installs `@bolt-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@bolt-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Bolt's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Bolt registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Bolt pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@bolt-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"bolt"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `bolt.default`.
...
- Local plugins persist installed themes under the local `.bolt/themes` area near the plugin config source.
...
- `--pure` / `BOLT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.bolt/plugins/tui-smoke.tsx`
- Local vim plugin: `.bolt/plugins/tui-vim.tsx`
- Local smoke config: `.bolt/tui.json`
- Local smoke theme: `.bolt/plugins/smoke-theme.json`
