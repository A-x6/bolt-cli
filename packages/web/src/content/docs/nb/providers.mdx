---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Bolt.
---

import config from "../../../../config.mjs"
export const console = config.console

Bolt bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Bolt-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i Bolt
konfig.
...
Bolt Zen er en liste over modeller levert av Bolt-teamet som har vært
testet og verifisert for å fungere godt med Bolt. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Bolt Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Bolt Zen` og gå til [bolt.ai/auth](https://bolt.ai/zen).
...
Det fungerer som alle andre leverandører i Bolt og er helt valgfritt å bruke.

---

## Bolt Go

Bolt Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Bolt-teamet som har vært
testet og verifisert for å fungere godt med Bolt.

1. Kjør kommandoen `/connect` i TUI, velg `Bolt Go`, og gå til [bolt.ai/auth](https://bolt.ai/zen).
...
Slik bruker du Amazon Bedrock med Bolt:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Bolt:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `bolt.json`:
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Bolt skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Bolt-konfigurasjonen.
...
for å lagre token i Bolt auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Bolt bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Bolt
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`bolt.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med Bolt:
...
Slik bruker du Google Vertex AI med Bolt:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Bolt, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Bolt-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Bolt-samtale oppført som en separat økt.
...
Du kan konfigurere Bolt for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Bolt for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Bolt for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Bolt. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Bolt:
...
3. Kopier API-nøkkelen for bruk i Bolt.
...
6. **Viktig**: Før du bruker skymodeller i Bolt, må du hente modellinformasjonen lokalt:
...
Bolt Zen er en liste over testede og verifiserte modeller levert av Bolt-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Bolt Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Bolt Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Bolt API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Bolt-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Bolt-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Bolt:
...
Du kan også tilpasse modeller gjennom Bolt-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Bolt. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `bolt.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Bolt forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
2. For tilpassede leverandører, sjekk Bolt-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Bolt-konfigurasjonen.
