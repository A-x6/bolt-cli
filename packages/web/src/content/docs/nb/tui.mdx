...
Bolt gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Bolt starter TUI for gjeldende katalog.
...
Når du bruker Bolt TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Bolt. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Dette er atskilt fra `bolt.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge Bolt venter etter leader key. Standard er `2000`.
...
Bruk `BOLT_TUI_CONFIG` for å laste en egendefinert TUI-konfigurasjonsbane.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `bolt.default`.
