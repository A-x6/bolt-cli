...
Det er to typer agenter i Bolt; primære agenter og underagenter.
...
Bolt kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Bolt kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Bolt kommer med to innebygde primære agenter og tre innebygde underagenter.
...
En skrivebeskyttet agent for ekstern dokumentasjon og forskning på avhengigheter. Bruk denne når du trenger å klone et avhengighetsrepo inn i Bolt sin administrerte cache, inspisere kildekoden til et bibliotek eller kryssjekke lokal kode mot upstream-implementasjoner uten å endre arbeidsområdet ditt.
...
Konfigurer agenter i konfigurasjonsfilen `bolt.json`:
...
Hvis ingen temperatur er spesifisert, bruker Bolt modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Bolt-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Bolt-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Bolt Zen](/docs/zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
