...
Du kan konfigurere Bolt ved å bruke en JSON konfigurasjonsfil.

---

## Format

Bolt støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "bolt"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
2. **Global konfigurasjon** (`~/.config/opencode/bolt.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`BOLT_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`bolt.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.bolt` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`BOLT_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.bolt` og `~/.config/opencode` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Plasser din globale Bolt-konfigurasjon i `~/.config/opencode/bolt.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `bolt.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Bolt starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `BOLT_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `BOLT_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.bolt` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.bolt`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`bolt.ai/config.json`**](https://bolt.ai/config.json).
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Bolt-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `bolt.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Bolt-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Bolt å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/opencode/agents/` eller `.bolt/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Bolt falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`bolt run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/opencode/commands/` eller `.bolt/commands/`. [Les mer her](/docs/commands).
...
Bolt vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Bolt alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Bolt med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.bolt/plugins/` eller `~/.config/opencode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Bolt til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
