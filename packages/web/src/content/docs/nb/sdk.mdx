---
title: SDK
description: Typesikker JS-klient for Bolt-server.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Bolt JS/TS SDK gir en typesikker klient for samhandling med serveren.
Bruk den til å bygge integrasjoner og kontrollere Bolt programmatisk.
...
Opprett en forekomst av Bolt:

```javascript
import { createBolt } from "@bolt-ai/sdk"

const { client } = await createBolt()
```
...
Du kan sende et konfigurasjonsobjekt for å tilpasse virkemåten. Forekomsten henter fortsatt din `bolt.json`, men du kan overstyre eller legge til konfigurasjon inline:
...
console.log(`Server running at ${bolt.server.url}`)
...
Hvis du allerede har en kjørende forekomst av Bolt, kan du opprette en klientforekomst for å koble til den:
...
```typescript
import type { Session, Message, Part } from "@bolt-ai/sdk"
```
