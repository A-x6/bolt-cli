...
For å feilsøke problemer med Bolt, start med å sjekke loggene og lokale data den lagrer på disken.
...
Bolt lagrer øktdata og andre applikasjonsdata på disken på:
...
Bolt Desktop kjører en lokal Bolt-server (`opencode-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.

### Raske sjekker

- Avslutt og start appen på nytt.
- Hvis appen viser en feilskjerm, klikker du på **Start på nytt** og kopierer feildetaljene.
- Bare macOS: `Bolt`-meny -> **Last nettvisning på nytt** (hjelper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/opencode/bolt.jsonc` (eller `~/.config/opencode/bolt.json`)
- **macOS/Linux** (eldre installasjoner): `~/.local/share/opencode/bolt.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\bolt.jsonc`
...
Bolt kan også laste lokale plugins fra disken. Flytt disse midlertidig ut av veien (eller gi nytt navn til mappen) og start skrivebordsappen på nytt:
...
Hvis deaktivering av plugins ikke hjelper (eller en plugin-installasjon sitter fast), tøm hurtigbufferen slik at Bolt kan gjenoppbygge den.

1. Avslutt Bolt Desktop helt.
...
3. Start Bolt Desktop på nytt.

---

### Løs problemer med servertilkobling

Bolt Desktop kan enten starte sin egen lokale server (standard) eller koble til en server URL du har konfigurert.
...
Hvis `bolt.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.

#### Sjekk miljøvariabler

Hvis du har `BOLT_PORT` satt i miljøet ditt, vil skrivebordsappen prøve å bruke den porten for den lokale serveren.

- Deaktiver `BOLT_PORT` (eller velg en ledig port) og start på nytt.
...
På Windows krever Bolt Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åpnes i et tomt vindu eller ikke starter, installer/oppdater WebView2 og prøv igjen.

---

### Windows: Generelle ytelsesproblemer

Hvis du opplever treg ytelse, problemer med filtilgang eller terminalproblemer på Windows, kan du prøve å bruke [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL gir et Linux-miljø som fungerer mer sømløst med funksjonene til Bolt.

---

### Varsler vises ikke

Bolt Desktop viser bare systemvarsler når:

- varsler er aktivert for Bolt i OS-innstillingene dine, og
...
1. Avslutt Bolt Desktop.
2. Finn og slett disse filene (de finnes i Bolt Desktop-appens datakatalog):

- `bolt.settings.dat` (stasjonær standardserver URL)
- `bolt.global.dat` og `bolt.workspace.*.dat` (UI tilstand som nylige servere/prosjekter)
...
Hvis du støter på API-anropsfeil, kan dette skyldes utdaterte provider-pakker. Bolt installerer dynamisk provider-pakker (OpenAI, Anthropic, Google, etc.) etter behov og cacher dem lokalt.
...
2. Start Bolt på nytt for å installere de nyeste provider-pakkene på nytt

Dette vil tvinge Bolt til å laste ned de nyeste versjonene av provider-pakkene, som ofte løser kompatibilitetsproblemer med modellparametere og API-endringer.
...
Bolt vil oppdage om du bruker Wayland og foretrekker `wl-clipboard`, ellers vil den prøve å finne utklippstavle-verktøy i rekkefølgen: `xclip` og `xsel`.
