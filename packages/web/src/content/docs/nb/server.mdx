---
title: Tjener
description: Samhandle med Bolt-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `opencode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Bolt-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `bolt.local` |
...
Sett `BOLT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `opencode` som standard, eller sett `BOLT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `opencode serve` og `opencode web`.

```bash
BOLT_SERVER_PASSWORD=your-password opencode serve
```
...
:::tip
Bruk Bolt-serveren til å samhandle med Bolt programmatisk.
:::

Denne arkitekturen lar Bolt støtte flere klienter og lar deg samhandle med Bolt programmatisk.

Du kan kjøre `opencode serve` for å starte en frittstående server. Hvis du har
Bolt TUI kjører, vil `opencode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Bolt [IDE](/docs/ide) plugins.
