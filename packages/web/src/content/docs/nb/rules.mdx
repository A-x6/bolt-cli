---
title: Regler
description: Set custom instructions for Bolt.
---

Du kan gi tilpassede instruksjoner for å Bolt ved å opprette en `AGENTS.md`-fil. Dette ligner på Cursors regler. Den inneholder instruksjoner som vil bli inkludert i LLMs kontekst for å tilpasse oppførselen til ditt spesifikke prosjekt.

---

## Initialisering

For å lage en ny `AGENTS.md` fil, kan du kjøre kommandoen `/init` i Bolt.

:::tip
Du bør sjekke inn prosjektets `AGENTS.md`-fil i Git.
:::

Dette vil skanne prosjektet og alt dets innhold for å forstå hva prosjektet handler om og generere en `AGENTS.md`-fil med det. Dette hjelper Bolt å navigere i prosjektet bedre.
...
Bolt støtter også lesing av `AGENTS.md`-filen fra flere steder. Og dette tjener forskjellige formål.
...
Du kan også ha globale regler i en `~/.config/opencode/AGENTS.md`-fil. Dette blir brukt på alle Bolt-økter.
...
For brukere som migrerer fra Claude Code, støtter Bolt Claude Codes filkonvensjoner som fallbacks:
...
```bash
export BOLT_DISABLE_CLAUDE_CODE=1        # Disable all .claude support
export BOLT_DISABLE_CLAUDE_CODE_PROMPT=1 # Disable only ~/.claude/CLAUDE.md
export BOLT_DISABLE_CLAUDE_CODE_SKILLS=1 # Disable only .claude/skills
```
...
Når Bolt starter, ser den etter regelfiler i denne rekkefølgen:
...
Du kan spesifisere egendefinerte instruksjonsfiler i din `bolt.json` eller den globale `~/.config/opencode/bolt.json`. Dette lar deg og teamet ditt gjenbruke eksisterende regler i stedet for å måtte duplisere dem til AGENTS.md.
...
Selv om Bolt ikke automatisk analyserer filreferanser i `AGENTS.md`, kan du oppnå lignende funksjonalitet på to måter:

### Bruker bolt.json

Den anbefalte tilnærmingen er å bruke feltet `instructions` i `bolt.json`:
...
Du kan lære Bolt å lese eksterne filer ved å gi eksplisitte instruksjoner i `AGENTS.md`. Her er et praktisk eksempel:
...
- Sørge for at Bolt laster filer kun når det er nødvendig for den spesifikke oppgaven

:::tip
For monorepos eller prosjekter med delte standarder er bruk av `bolt.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
