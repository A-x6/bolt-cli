...
description: Skriv dine egne programtillegg for å utvide Bolt.
---

Plugins lar deg utvide Bolt ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Bolt.
...
- `.bolt/plugins/` - Programtillegg på prosjektnivå
...
1. Global konfigurasjon (`~/.config/opencode/bolt.json`)
2. Prosjektkonfigurasjon (`bolt.json`)
3. Global plugin-katalog (`~/.config/opencode/plugins/`)
4. Prosjektpluginkatalog (`.bolt/plugins/`)
...
Bolt kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Bolt SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Bolt.
...
:::note
Hvis du bruker Bolt-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Bolt fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Bolt:
...
`tool`-hjelperen lager et tilpasset verktøy som Bolt kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Bolt sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://bolt.ai/docs/sdk) for detaljer.
