---
title: المزوّدون
description: استخدام أي موفّر LLM في Bolt.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Bolt ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Bolt.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Bolt.
...
Bolt Zen هي قائمة نماذج يوفّرها فريق Bolt وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Bolt. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Bolt Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opencode، ثم انتقل إلى [bolt.ai/auth](https://bolt.ai/auth).
...
يعمل مثل أي مزوّد آخر في Bolt واستخدامه اختياري بالكامل.

---

## Bolt Go

Bolt Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Bolt، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Bolt.

1. شغّل الأمر `/connect` في TUI، واختر `Bolt Go`، ثم انتقل إلى [bolt.ai/auth](https://bolt.ai/zen).
...
لاستخدام Amazon Bedrock مع Bolt:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `bolt.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Bolt غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات bolt.
...
تخزين الرمز في مساحة تخزين مصادقة bolt.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Bolt نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Bolt
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`bolt.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام Google Vertex AI مع Bolt:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Bolt تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Bolt مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Bolt. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في Bolt.
...
6. **مهم**: قبل استخدام نماذج السحابة في Bolt، يجب سحب معلومات النموذج محليا:
...
Bolt Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Bolt. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Bolt Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Bolt Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Bolt.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات bolt.
...
5. يمكنك أيضا تخصيصها عبر إعدادات bolt. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات bolt. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع bolt. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `bolt.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Bolt بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
2. للمزوّدات المخصّصة، راجع إعدادات opencode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات bolt.
