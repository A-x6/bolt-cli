---
title: القواعد
description: اضبط تعليمات مخصصة لـ bolt.
---
...
لإنشاء ملف `AGENTS.md` جديد، يمكنك تشغيل الأمر `/init` في bolt.
...
يمكنك أيضًا وضع قواعد عامة في ملف `~/.config/opencode/AGENTS.md`. تُطبَّق هذه القواعد على جميع جلسات bolt.
...
للمستخدمين الذين ينتقلون من Claude Code، يدعم Bolt اصطلاحات ملفات Claude Code كبدائل احتياطية:
...
يمكنك تحديد ملفات تعليمات مخصصة في `bolt.json` أو في الملف العام `~/.config/opencode/bolt.json`. يتيح ذلك لك ولفريقك إعادة استخدام القواعد الموجودة بدلًا من تكرارها داخل AGENTS.md.
...
النهج الموصى به هو استخدام الحقل `instructions` في `bolt.json`:
...
:::tip
بالنسبة للمستودعات متعددة الحزم (monorepos) أو المشاريع ذات المعايير المشتركة، فإن استخدام `bolt.json` مع أنماط glob (مثل `packages/*/AGENTS.md`) أسهل للصيانة من التعليمات اليدوية.
:::
