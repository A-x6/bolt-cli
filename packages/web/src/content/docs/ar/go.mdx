...
Bolt Go هو اشتراك منخفض التكلفة بقيمة **$10/شهريًا** يمنحك وصولًا موثوقًا إلى نماذج البرمجة المفتوحة الشائعة.

يعمل Go مثل أي مزود آخر في Bolt. تشترك في Bolt Go وتحصل على مفتاح API الخاص بك. وهو **اختياري تمامًا**، ولا تحتاج إلى استخدامه لاستخدام Bolt.
...
:::tip
اختبرنا مجموعة مختارة من النماذج والمزودين الذين يعملون جيدًا مع Bolt.
:::
...
يمنحك Bolt Go الوصول إلى هذه النماذج مقابل **$10/شهريًا**.

---

## كيف يعمل

يعمل Bolt Go مثل أي مزود آخر في Bolt.

1. تسجّل الدخول إلى **<a href={console}>Bolt Zen</a>**، وتشترك في Go، ثم تنسخ مفتاح API الخاص بك.
2. تشغّل الأمر `/connect` في TUI، وتختار `Bolt Go`، ثم تلصق مفتاح API الخاص بك.
3. شغّل `/models` في TUI لرؤية قائمة النماذج المتاحة عبر Go.

:::note
يمكن لعضو واحد فقط في كل workspace الاشتراك في Bolt Go.
:::
...
صُمّم Bolt Go للاستخدام مع [Bolt](https://bolt.ai) ووكلاء البرمجة الآخرين
...
بالإضافة إلى Bolt، تم التحقق من أن العملاء التاليين يعملون بصورة صحيحة
مع Bolt Go، مع أننا لا نضمن استمرارهم في العمل مستقبلًا.

| العميل            | دعم الجلسات                                                                                                                                                                                                                      |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | ترسل البُنى التي تتضمن [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) الترويسة في طلبات Bolt الرئيسية والمساعدة. دُمج الإصلاح بعد v0.21.0؛ ولا يتضمن ذلك الإصدار وحده الإصلاح.                       |
...
| **Pi**            | ترسل الإصدارات الحالية معلومات الجلسة إلى Bolt. حدّث عمليات التثبيت الأقدم.                                                                                                                                                  |
| **jcode**         | حدّث إلى **v0.81.6 أو إصدار أحدث**، إذ يتضمن [إصلاح ترويسة الجلسة](https://github.com/1jehuang/jcode/issues/1167).                                                                                                               |
| **Kilo Code CLI** | تعيد الإصدارات التي تتضمن [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) ترويسات جلسات Bolt. يشمل هذا الإصلاح CLI، وليس إضافة VS Code. راجع [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
يستخدم [model id](/docs/config/#models) في إعدادات Bolt لديك التنسيق `opencode-go/<model-id>`. على سبيل المثال، بالنسبة إلى Kimi K3، ستستخدم `opencode-go/kimi-k3` في إعداداتك.
...
4. **عدم فرض قيود ارتباط** من خلال السماح لك أيضًا باستخدام أي مزود آخر مع Bolt.
