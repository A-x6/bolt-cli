---
title: IDE
description: امتداد Bolt لـ VS Code و Cursor وغيرها من بيئات التطوير
---

يتكامل Bolt مع VS Code و Cursor أو أي IDE يدعم terminal. ما عليك سوى تشغيل `opencode` في terminal للبدء.

---

## الاستخدام

- **تشغيل سريع**: استخدم `Cmd+Esc` (Mac) أو `Ctrl+Esc` (Windows/Linux) لفتح Bolt في عرض terminal مقسّم، أو للتركيز على جلسة terminal موجودة إذا كانت قيد التشغيل بالفعل.
- **جلسة جديدة**: استخدم `Cmd+Shift+Esc` (Mac) أو `Ctrl+Shift+Esc` (Windows/Linux) لبدء جلسة terminal جديدة لـ Bolt حتى لو كانت هناك جلسة مفتوحة. يمكنك أيضا النقر على زر Bolt في واجهة المستخدم.
- **وعي بالسياق**: شارك تلقائيا تحديدك الحالي أو تبويبك مع Bolt.
...
لتثبيت Bolt على VS Code والتفرعات الشائعة مثل Cursor و Windsurf و VSCodium:
...
ابحث عن **Bolt** في Extension Marketplace وانقر **Install**.
