...
يمكنك إضافة أدوات خارجية إلى Bolt باستخدام _Model Context Protocol_، أو MCP. يدعم Bolt خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Bolt](https://bolt.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يتولى Bolt تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Bolt بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Bolt المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `opencode mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Bolt الرموز بشكل آمن في `~/.local/share/opencode/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Bolt إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Bolt مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Bolt بحساب Sentry الخاص بك.
