...
| `--mdns-domain` | Custom domain name for mDNS service | `bolt.local` |
...
Set `BOLT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opencode`, or set `BOLT_SERVER_USERNAME` to override it. This applies to both `opencode serve` and `opencode web`.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Bolt [IDE](/docs/ide) plugins.
