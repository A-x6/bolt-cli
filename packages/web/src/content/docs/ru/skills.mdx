...
- Конфигурация проекта: `.bolt/skills/<name>/SKILL.md`
...
Он загружает все соответствующие `skills/*/SKILL.md` в `.bolt/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.
...
Создайте `.bolt/skills/git-release/SKILL.md` следующим образом:
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `bolt.json`:
...
**Для встроенных агентов** (в формате `bolt.json`):
