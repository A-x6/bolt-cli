...
Установите Bolt из [реестра ACP Zed](https://zed.dev/docs/ai/external-agents#registry), выполнив `zed: acp registry` в палитре команд.

Чтобы вместо этого использовать собственный исполняемый файл Bolt, добавьте его в конфигурацию [Zed](https://zed.dev) (`~/.config/zed/settings.json`):
...
Если вам нужно передать переменные среды (например, `BOLT_API_KEY`), обратитесь к разделу [Настройка адаптеров: переменные среды](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) в документации CodeCompanion.nvim для получения полной информации.
