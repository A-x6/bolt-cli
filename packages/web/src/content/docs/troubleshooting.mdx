...
To debug issues with Bolt, start by checking the logs and local data it stores on disk.
...
To uninstall the Bolt CLI and remove its related files, run:
...
To remove Bolt Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

Bolt Desktop runs a local Bolt server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Bolt` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/opencode/bolt.jsonc` (or `~/.config/opencode/bolt.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/bolt.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\bolt.jsonc`
...
Bolt can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Bolt can rebuild it.

1. Quit Bolt Desktop completely.
...
3. Restart Bolt Desktop.

---

### Fix server connection issues

Bolt Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `bolt.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `BOLT_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `BOLT_PORT` (or pick a free port) and restart.
...
On Windows, Bolt Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with Bolt's features.

---

### Notifications not showing

Bolt Desktop only shows system notifications when:

- notifications are enabled for Bolt in your OS settings, and
...
1. Quit Bolt Desktop.
2. Find and delete these files (they live in the Bolt Desktop app data directory):

- `bolt.settings.dat` (desktop default server URL)
- `bolt.global.dat` and `bolt.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with Bolt:
