---
title: TUI
description: Using the Bolt terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Bolt provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Bolt starts the TUI for the current directory.
...
When using the Bolt TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Bolt. Allows you to select from available providers and add their API keys.
...
This is separate from `bolt.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Bolt waits after the leader key. Defaults to `2000`.
...
Use `BOLT_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `bolt.default`.
