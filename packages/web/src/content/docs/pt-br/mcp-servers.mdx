...
Você pode definir servidores MCP em sua [Configuração do opencode](https://bolt.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
Seus MCPs estão disponíveis como ferramentas no bolt, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do opencode como qualquer outra ferramenta.
