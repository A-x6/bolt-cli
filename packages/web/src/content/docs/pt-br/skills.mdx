...
- Configuração do projeto: `.bolt/skills/<name>/SKILL.md`
...
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.bolt/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.
...
Crie `.bolt/skills/git-release/SKILL.md` assim:
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `bolt.json`:
...
**Para agentes embutidos** (em `bolt.json`):
