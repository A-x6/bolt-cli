...
description: Crie ferramentas que o LLM pode chamar no bolt.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do bolt, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.bolt/tools/` do seu projeto.
...
```ts title=".bolt/tools/database.ts" {1}
...
```ts title=".bolt/tools/project.ts" {8}
import { tool } from "@bolt-ai/plugin"
...
```python title=".bolt/tools/add.py"
import sys
...
```ts title=".bolt/tools/python-add.ts" {10}
import { tool } from "@bolt-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".bolt/tools/add.py")
