---
title: Regras
description: Defina instruções personalizadas para bolt.
---
...
Para criar um novo arquivo `AGENTS.md`, você pode executar o comando `/init` no bolt.
...
Você também pode ter regras globais em um arquivo `~/.config/opencode/AGENTS.md`. Isso é aplicado em todas as sessões do bolt.
...
Você pode especificar arquivos de instrução personalizados no seu `bolt.json` ou no global `~/.config/opencode/bolt.json`. Isso permite que você e sua equipe reutilizem regras existentes em vez de ter que duplicá-las no AGENTS.md.
...
A abordagem recomendada é usar o campo `instructions` em `bolt.json`:
...
:::tip
Para monorepos ou projetos com padrões compartilhados, usar `bolt.json` com padrões glob (como `packages/*/AGENTS.md`) é mais sustentável do que instruções manuais.
:::
