...
Para depurar problemas com o bolt, comece verificando os logs e os dados locais que ele armazena no disco.
...
- Apenas macOS: menu `Bolt` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/opencode/bolt.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\bolt.jsonc`
...
Se seu `bolt.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `BOLT_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `BOLT_PORT` (ou escolha uma porta livre) e reinicie.
...
Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do bolt.
...
- `bolt.settings.dat` (URL do servidor padrão do desktop)
- `bolt.global.dat` e `bolt.workspace.*.dat` (estado da interface como servidores/projetos recentes)
