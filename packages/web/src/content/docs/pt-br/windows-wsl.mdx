...
Embora o opencode possa rodar direto no Windows, recomendamos usar [Windows Subsystem for Linux (WSL)](https://learn.microsoft.com/en-us/windows/wsl/install) para a melhor experiência. O WSL oferece um ambiente Linux que funciona de forma integrada com os recursos do bolt.
...
3. **Use o opencode pelo WSL**
   ...
   Vá para o diretório do seu projeto (acesse arquivos do Windows via `/mnt/c/`, `/mnt/d/` etc.) e execute o bolt.
...
Se você prefere usar o app desktop do bolt, mas quer rodar o servidor no WSL:
...
:::caution
Ao usar `--hostname 0.0.0.0`, defina `BOLT_SERVER_PASSWORD` para proteger o servidor.
