...
- **Nova Sessão**: Use `Cmd+Shift+Esc` (Mac) ou `Ctrl+Shift+Esc` (Windows/Linux) para iniciar uma nova sessão de terminal bolt, mesmo que uma já esteja aberta. Você também pode clicar no botão opencode na interface.
- **Consciência de Contexto**: Compartilhe automaticamente sua seleção ou aba atual com o bolt.
