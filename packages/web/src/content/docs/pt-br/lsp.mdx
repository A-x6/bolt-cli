...
:::note
Você pode desabilitar os downloads automáticos do servidor LSP definindo a variável de ambiente `BOLT_DISABLE_LSP_DOWNLOAD` como `true`.
:::
...
Você pode habilitar e personalizar os servidores LSP através da seção `lsp` na sua configuração do bolt.
