...
2. **Adicione o fluxo de trabalho**
   ...
   Adicione o seguinte arquivo de fluxo de trabalho em `.github/workflows/bolt.yml` no seu repositório. Certifique-se de definir o `model` apropriado e as chaves de API necessárias em `env`.
...
- `model`: O modelo a ser usado com o bolt. Tem o formato de `provider/model`. Isso é **obrigatório**.
...
- `share`: Se deve compartilhar a sessão do bolt. O padrão é **true** para repositórios públicos.
...
- `token`: Token de acesso do GitHub opcional para realizar operações como criar comentários, confirmar alterações e abrir pull requests. Por padrão, o opencode usa o token de acesso da instalação do aplicativo GitHub bolt, então commits, comentários e pull requests aparecem como se fossem da aplicação.

  Alternativamente, você pode usar o [`GITHUB_TOKEN`](https://docs.github.com/en/actions/tutorials/authenticate-with-github_token) embutido do runner do GitHub Action sem instalar o aplicativo GitHub bolt. Apenas certifique-se de conceder as permissões necessárias em seu fluxo de trabalho:
