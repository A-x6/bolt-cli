---
title: CLI
description: Opções e comandos da CLI do bolt.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do bolt, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.
...
Inicie a interface de usuário do terminal do bolt.
...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `BOLT_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `BOLT_SERVER_USERNAME` ou `opencode`) |
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no bolt.
...
```bash "bolt run"
bolt run Explique o uso de context em Go
```
...
bolt run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
Isso inicia um servidor HTTP que fornece acesso à funcionalidade do opencode sem a interface TUI. Defina `BOLT_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
Liste todas as sessões do bolt.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do bolt.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do bolt.

```bash
opencode import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do bolt.
...
Isso inicia um servidor HTTP e abre um navegador para acessar o opencode através de uma interface web. Defina `BOLT_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `BOLT_AUTO_SHARE`                 | boolean | Compartilhar sessões automaticamente                                  |
| `BOLT_GIT_BASH_PATH`              | string  | Caminho para o executável do Git Bash no Windows                      |
| `BOLT_CONFIG`                     | string  | Caminho para o arquivo de configuração                                |
| `BOLT_CONFIG_DIR`                 | string  | Caminho para o diretório de configuração                              |
| `BOLT_CONFIG_CONTENT`             | string  | Conteúdo de configuração json inline                                  |
| `BOLT_DISABLE_AUTOUPDATE`         | boolean | Desabilitar verificações de atualização automática                    |
| `BOLT_DISABLE_PRUNE`              | boolean | Desabilitar a poda de dados antigos                                   |
| `BOLT_DISABLE_TERMINAL_TITLE`     | boolean | Desabilitar atualizações automáticas do título do terminal            |
| `BOLT_PERMISSION`                 | string  | Configuração de permissões json inline                                |
| `BOLT_DISABLE_DEFAULT_PLUGINS`    | boolean | Desabilitar plugins padrão                                            |
| `BOLT_DISABLE_LSP_DOWNLOAD`       | boolean | Desabilitar downloads automáticos do servidor LSP                     |
| `BOLT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Habilitar modelos experimentais                                       |
| `BOLT_DISABLE_AUTOCOMPACT`        | boolean | Desabilitar compactação automática de contexto                        |
| `BOLT_DISABLE_CLAUDE_CODE`        | boolean | Desabilitar leitura de `.claude` (prompt + habilidades)               |
| `BOLT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Desabilitar leitura de `~/.claude/CLAUDE.md`                          |
| `BOLT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Desabilitar carregamento de `.claude/skills`                          |
| `BOLT_DISABLE_MODELS_FETCH`       | boolean | Desabilitar busca de modelos de fontes remotas                        |
| `BOLT_FAKE_VCS`                   | string  | Provedor VCS falso para fins de teste                                 |
| `BOLT_CLIENT`                     | string  | Identificador do cliente (padrão é `cli`)                             |
| `BOLT_ENABLE_EXA`                 | boolean | Habilitar ferramentas de busca web Exa                                |
| `BOLT_SERVER_PASSWORD`            | string  | Habilitar autenticação básica para `serve`/`web`                      |
| `BOLT_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `opencode`) |
| `BOLT_MODELS_URL`                 | string  | URL personalizada para buscar configuração de modelos                 |
...
| Variável                                        | Tipo    | Descrição                                                 |
| ----------------------------------------------- | ------- | --------------------------------------------------------- |
| `BOLT_EXPERIMENTAL`                         | boolean | Habilitar recursos experimentais sob o sinalizador global |
| `BOLT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Habilitar descoberta de ícones                            |
| `BOLT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Desabilitar cópia ao selecionar no TUI                    |
| `BOLT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Tempo limite padrão para comandos bash em ms              |
| `BOLT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Máximo de tokens de saída para respostas LLM              |
| `BOLT_EXPERIMENTAL_FILEWATCHER`             | boolean | Habilitar monitoramento de arquivos para todo o diretório |
| `BOLT_EXPERIMENTAL_OXFMT`                   | boolean | Habilitar formatador oxfmt                                |
| `BOLT_EXPERIMENTAL_LSP_TOOL`                | boolean | Habilitar ferramenta LSP experimental                     |
| `BOLT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Desabilitar monitoramento de arquivos                     |
| `BOLT_EXPERIMENTAL_EXA`                     | boolean | Habilitar recursos experimentais do Exa                   |
| `BOLT_EXPERIMENTAL_LSP_TY`                  | boolean | Habilitar TY LSP para arquivos python                     |
| `BOLT_EXPERIMENTAL_PLAN_MODE`               | boolean | Habilitar modo de plano                                   |
| `BOLT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Habilitar tarefas de subagentes em segundo plano          |
| `BOLT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Habilitar sistema de eventos experimental                 |
| `BOLT_EXPERIMENTAL_NATIVE_LLM`              | boolean | Habilitar caminho nativo de requisição LLM                |
| `BOLT_EXPERIMENTAL_PARALLEL`                | boolean | Habilitar execução paralela de busca web                  |
| `BOLT_EXPERIMENTAL_SCOUT`                   | boolean | Habilitar subagente Scout                                 |
| `BOLT_EXPERIMENTAL_WORKSPACES`              | boolean | Habilitar suporte a espaços de trabalho                   |
