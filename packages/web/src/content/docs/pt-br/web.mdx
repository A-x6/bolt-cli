...
:::caution
Se `BOLT_SERVER_PASSWORD` não estiver definido, o servidor ficará sem segurança. Isso é aceitável para uso local, mas deve ser configurado para acesso à rede.
:::
...
Isso define automaticamente o nome do host como `0.0.0.0` e anuncia o servidor como `bolt.local`.
...
Para proteger o acesso, defina uma senha usando a variável de ambiente `BOLT_SERVER_PASSWORD`:

```bash
BOLT_SERVER_PASSWORD=secret opencode web
```

O nome de usuário padrão é `opencode`, mas pode ser alterado com `BOLT_SERVER_USERNAME`.

---

## Usando a Interface Web

Uma vez iniciada, a interface web fornece acesso às suas sessões do bolt.
...
Você também pode configurar as configurações do servidor no seu arquivo de configuração `bolt.json`:
