---
title: Configuração
description: Usando a configuração JSON do bolt.
---
...
2. **Configuração global** (`~/.config/opencode/bolt.json`) - preferências do usuário
3. **Configuração personalizada** (`BOLT_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`bolt.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.bolt`** - agentes, comandos, plugins
6. **Configuração inline** (`BOLT_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.bolt` e `~/.config/opencode` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do opencode em `~/.config/opencode/bolt.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Adicione `bolt.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `BOLT_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `BOLT_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.bolt`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.bolt`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`bolt.ai/config.json`**](https://bolt.ai/config.json).

A configuração do TUI usa [**`bolt.ai/tui.json`**](https://bolt.ai/tui.json).
...
Use `BOLT_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `bolt.json` estão obsoletas e são migradas automaticamente quando possível.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor bolt.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `bolt.local`. Útil para executar várias instâncias na mesma rede.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/opencode/agents/` ou `.bolt/agents/`. [Saiba mais aqui](/docs/agents).
...
Essa configuração se aplica a todas as interfaces: TUI, CLI (`bolt run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/opencode/commands/` ou `.bolt/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.bolt/plugins/` ou `~/.config/opencode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
