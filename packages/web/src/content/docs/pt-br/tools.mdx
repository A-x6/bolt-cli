...
Aqui estão todas as ferramentas integradas disponíveis no bolt.
...
:::note
Esta ferramenta está disponível apenas quando `BOLT_EXPERIMENTAL_LSP_TOOL=true` (ou `BOLT_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor opencode ou quando a variável de ambiente `BOLT_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).
