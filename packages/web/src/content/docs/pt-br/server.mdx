...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `bolt.local` |
...
Defina `BOLT_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `opencode`, ou defina `BOLT_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `opencode serve` quanto ao `opencode web`.
