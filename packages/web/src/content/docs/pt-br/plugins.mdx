---
title: Plugins
description: Escreva seus próprios plugins para estender o bolt.
---

Plugins permitem que você estenda o opencode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do bolt.
...
- `.bolt/plugins/` - Plugins em nível de projeto
...
1. Configuração global (`~/.config/opencode/bolt.json`)
2. Configuração do projeto (`bolt.json`)
3. Diretório de plugins global (`~/.config/opencode/plugins/`)
4. Diretório de plugins do projeto (`.bolt/plugins/`)
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o bolt.
...
:::note
Se você estiver usando o aplicativo desktop bolt, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://bolt.ai/docs/sdk) para detalhes.
