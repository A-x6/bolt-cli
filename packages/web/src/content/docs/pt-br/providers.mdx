---
title: Provedores
description: Usando qualquer provedor LLM no bolt.
---
...
2. Configurar o provedor na sua configuração do bolt.
...
Você pode personalizar os provedores através da seção `provider` na sua configuração do bolt.
...
Bolt Zen é uma lista de modelos fornecidos pela equipe do opencode que foram testados e verificados para funcionar bem com o bolt. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Bolt Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `Bolt Zen` e acesse [bolt.ai/zen](https://bolt.ai/zen).
...
Bolt Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do opencode que foram
testados e verificados para funcionar bem com o bolt.

1. Execute o comando `/connect` no TUI, selecione `Bolt Go` e acesse [bolt.ai/zen](https://bolt.ai/zen).
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `bolt.json`:
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do bolt.
...
armazenar o token no armazenamento de autenticação do bolt.
:::
...
arquivo `bolt.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `bolt.json`:
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do bolt, você sempre pode configurá-lo você mesmo.
...
:::tip
Ollama pode se configurar automaticamente para o bolt. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/opencode) para detalhes.
:::
...
3. Copie a chave da API para uso no bolt.
...
6. **Importante**: Antes de usar modelos em nuvem no bolt, você deve puxar as informações do modelo localmente:
...
Bolt Zen é uma lista de modelos testados e verificados fornecidos pela equipe do bolt. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Bolt Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Bolt Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do bolt.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do bolt.
...
5. Você também pode personalizá-los através da sua configuração do bolt. Aqui está um exemplo de especificar um provedor
...
Você também pode personalizar modelos através da sua configuração do bolt. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o bolt. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no bolt.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `bolt.json` no diretório do seu projeto:
...
2. Para provedores personalizados, verifique a configuração do opencode e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do bolt.
