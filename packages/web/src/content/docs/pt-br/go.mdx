...
O Bolt Go é uma assinatura de baixo custo de **US$ 10/mês** que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Bolt. Você assina o Bolt Go e
...
O Bolt Go lhe dá acesso a esses modelos por **US$ 10/mês**.

---

## Como funciona

O Bolt Go funciona como qualquer outro provedor no Bolt.

1. Você entra no **<a href={console}>Bolt Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Bolt Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Bolt Go.
:::
...
O Bolt Go foi projetado para o [Bolt](https://bolt.ai) e outros agentes de programação
...
Além do Bolt, os clientes a seguir foram validados para funcionar corretamente
com o Bolt Go. No entanto, não garantimos que continuarão funcionando no futuro.

| Cliente           | Suporte a sessões                                                                                                                                                                                                                                                     |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | As builds que contêm o [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) enviam o header nas requisições principais e auxiliares do Bolt. A correção foi incorporada após a v0.21.0; essa versão por si só não a inclui.                     |
...
| **Pi**            | As builds atuais enviam informações de sessão para o Bolt. Atualize instalações mais antigas.                                                                                                                                                                     |
...
| **Kilo Code CLI** | As builds que contêm o [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) restauram os headers de sessão do Bolt. Essa correção abrange a CLI, não a extensão do VS Code. Consulte a [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
O [ID do modelo](/docs/config/#models) na sua configuração do Bolt
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Bolt também.
