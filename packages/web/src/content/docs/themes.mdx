...
With Bolt you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Bolt uses our own `opencode` theme.
...
Bolt comes with several built-in themes.
...
- Want Bolt to match their terminal's appearance
...
Bolt supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
3. **Project root directory** - Defined in the `<project-root>/.bolt/themes/*.json`
4. **Current working directory** - Defined in `./.bolt/themes/*.json`
