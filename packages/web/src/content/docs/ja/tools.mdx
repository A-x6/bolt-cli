...
ツールを使用すると、LLM がコードベースでアクションを実行できるようになります。 Bolt には一連の組み込みツールが付属していますが、[カスタムツール](/docs/custom-tools) または [MCP サーバー](/docs/mcp-servers) を使用して拡張できます。
...
:::note
このツールは、`BOLT_EXPERIMENTAL_LSP_TOOL=true` (または `BOLT_EXPERIMENTAL=true`) の場合にのみ使用できます。
:::
...
:::note
このツールは、Bolt プロバイダーを使用している場合、または `BOLT_ENABLE_EXA` 環境変数が真実の値 (`true` または `1` など) に設定されている場合にのみ使用できます。
