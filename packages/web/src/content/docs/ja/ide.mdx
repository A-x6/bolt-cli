---
title: IDE
description: VS Code、Cursor、およびその他の IDE 用の Bolt 拡張機能
---

Bolt は、VS Code、Cursor、またはターミナルをサポートする任意の IDE と統合します。開始するには、ターミナルで `opencode` を実行するだけです。

---

## 使用法

- **クイック起動**: `Cmd+Esc` (Mac) または `Ctrl+Esc` (Windows/Linux) を使用して、分割ターミナルビューで Bolt を開くか、既存のターミナルセッションが既に実行されている場合はそれにフォーカスします。
- **新しいセッション**: すでに開いている場合でも、`Cmd+Shift+Esc` (Mac) または `Ctrl+Shift+Esc` (Windows/Linux) を使用して、新しい Bolt ターミナルセッションを開始します。 UI の [Bolt] ボタンをクリックすることもできます。
- **コンテキスト認識**: 現在の選択またはタブを Bolt と自動的に共有します。
...
Bolt を VS Code および Cursor、Windsurf、VSCodium などの一般的なフォークにインストールするには:
...
Extension Marketplace で **Bolt** を検索し、**インストール** をクリックします。
