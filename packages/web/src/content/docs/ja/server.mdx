---
title: サーバー
description: HTTP 経由で Bolt サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`opencode serve` コマンドは、Bolt クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `bolt.local` |
...
HTTP 基本認証でサーバーを保護するには、`BOLT_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `opencode` になるか、`BOLT_SERVER_USERNAME` を設定してオーバーライドします。これは、`opencode serve` と `opencode web` の両方に当てはまります。
...
:::tip
Bolt サーバーを使用して、プログラムで Bolt と対話します。
:::

`opencode serve` を実行してスタンドアロンサーバーを起動できます。すでに Bolt TUI を実行している場合でも、`opencode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Bolt [IDE](/docs/ide) プラグインによって使用されます。
