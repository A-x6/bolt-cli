---
title: プラグイン
description: Bolt を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Bolt を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Bolt のデフォルトの動作を変更したりできます。
...
1. グローバル設定 (`~/.config/opencode/bolt.json`)
2. プロジェクト設定 (`bolt.json`)
3. グローバルプラグインディレクトリ (`~/.config/opencode/plugins/`)
4. プロジェクトプラグインディレクトリ (`.bolt/plugins/`)
...
Bolt は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Bolt SDK クライアント。
...
ここでは、Bolt を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://bolt.ai/docs/sdk) を参照してください。
...
プラグインは Bolt にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Bolt が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Bolt で使用できます。
