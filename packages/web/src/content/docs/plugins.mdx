---
title: Plugins
description: Write your own plugins to extend Bolt.
---

Plugins allow you to extend Bolt by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify Bolt's default behavior.
...
- `.bolt/plugins/` - Project-level plugins
...
4. Project plugin directory (`.bolt/plugins/`)
...
Bolt runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
Here are some examples of plugins you can use to extend bolt.
...
:::note
If you’re using the Bolt desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://bolt.ai/docs/sdk) for details.
