...
Setze `BOLT_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `opencode`, kann aber mit `BOLT_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `opencode serve` und `opencode web`.
...
Dieses Muster nutzen auch die Bolt-[IDE](/docs/ide)-Plugins.
