...
Agent Skills erlauben Bolt, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Bolt sucht in folgenden Pfaden:

- Project config: `.bolt/skills/<name>/SKILL.md`
...
Bei projektlokalen Pfaden laeuft Bolt vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.bolt/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.
...
Erstelle `.bolt/skills/git-release/SKILL.md` zum Beispiel so:
...
Bolt listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `bolt.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `bolt.json`):
