...
Es gibt zwei Arten von Agenten in Bolt; Primäragenten und Subagenten.
...
Bolt verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `bolt.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Bolt modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Bolt-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Bolt-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Bolt Zen](/docs/zen) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
