...
Sie können externe Tools zu Bolt hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Bolt unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Bolt Config](https://bolt.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Bolt übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Bolt Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Bolt beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `opencode mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Bolt die Token sicher in `~/.local/share/opencode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Bolt verfügbar. Sie können sie also wie jedes andere Tool über die Bolt-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Bolt mit Ihrem Sentry-Konto zu verbinden.
