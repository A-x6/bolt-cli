...
Wenn Bolt Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Bolt Desktop startet im Hintergrund einen lokalen Bolt-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `Bolt`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/opencode/bolt.jsonc` (oder `~/.config/opencode/bolt.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opencode/bolt.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\bolt.jsonc`
...
Bolt kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Bolt ihn neu aufbauen kann.

1. Beende Bolt Desktop komplett.
...
3. Starte Bolt Desktop neu.

---

### Server-Verbindungsprobleme beheben

Bolt Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `bolt.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `BOLT_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `BOLT_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Bolt Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Bolt-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Bolt Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Bolt in den OS-Einstellungen aktiviert sind, und
...
1. Beende Bolt Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Bolt Desktop):

- `bolt.settings.dat` (desktop default server URL)
- `bolt.global.dat` und `bolt.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Bolt hast:
...
Dies zwingt bolt, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
