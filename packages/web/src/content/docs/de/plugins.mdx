---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Bolt zu erweitern.
---

Mit Plugins können Sie Bolt erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Bolt zu ändern.
...
- `.bolt/plugins/` – Plugins auf Projektebene
...
```json title="bolt.json"
{
  ...
  "plugin": ["bolt-helicone-session", "bolt-wakatime", "@my-org/custom-plugin"]
}
```
...
1. Globale Konfiguration (`~/.config/opencode/bolt.json`)
2. Projektkonfiguration (`bolt.json`)
3. Globales Plugin-Verzeichnis (`~/.config/opencode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.bolt/plugins/`)
...
Bolt führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.

```ts title=".bolt/plugins/my-plugin.ts"
...
```js title=".bolt/plugins/example.js"
...
- `client`: Ein Bolt-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Bolt erweitern können.
...
```js title=".bolt/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "bolt"'`
      }
    },
  }
}
...
:::note
Wenn Sie die Bolt-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Bolt `.env`-Dateien liest:

```javascript title=".bolt/plugins/env-protection.js"
...
```javascript title=".bolt/plugins/inject-env.js"
...
Plugins können Bolt auch benutzerdefinierte Tools hinzufügen:

```ts title=".bolt/plugins/custom-tools.ts"
import { type Plugin, tool } from "@bolt-ai/plugin"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Bolt aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Bolt zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://bolt.ai/docs/sdk).
...
```ts title=".bolt/plugins/compaction.ts"
import type { Plugin } from "@bolt-ai/plugin"
...
```ts title=".bolt/plugins/custom-compaction.ts"
import type { Plugin } from "@bolt-ai/plugin"
