---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Bolt.
---

import config from "../../../../config.mjs"
export const console = config.console

Bolt verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Bolt-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Bolt anpassen
config.
...
Bolt Zen ist eine Liste von Modellen, die vom Bolt-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Bolt funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Bolt Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Bolt aus und gehen Sie zu [bolt.ai/auth](https://bolt.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Bolt und ist völlig optional.

---

## Bolt Go

Bolt Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Bolt-Team getestet und verifiziert wurden, dass sie gut mit Bolt funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Bolt Go` und gehen Sie zu [bolt.ai/auth](https://bolt.ai/zen).
...
So verwenden Sie Amazon Bedrock mit Bolt:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Bolt ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `bolt.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Bolt wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Bolt ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Bolt-Konfiguration hinzufügen.
...
um das Token im Bolt-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Bolt verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Bolt
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`bolt.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Bolt:
...
So verwenden Sie Google Vertex AI mit Bolt:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Bolt ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Bolt konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede Bolt-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Bolt-Konversation als separate Sitzung aufgeführt.
...
Sie können Bolt für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Bolt über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Bolt über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Bolt konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit Bolt:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Bolt.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Bolt verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Bolt Zen ist eine Liste getesteter und verifizierter Modelle, die vom Bolt-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Bolt Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Bolt Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Bolt API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Bolt-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Bolt-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Bolt:
...
Sie können Modelle auch über Ihre Bolt-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Bolt verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `bolt.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Bolt erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Bolt-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Bolt-Konfiguration übereinstimmt.
