---
title: TUI
description: Arbeite mit Bolt in der Terminal-Oberflaeche.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Bolt bietet eine interaktive Terminaloberflaeche (TUI), um mit einem LLM an deinen Projekten zu arbeiten.

Ohne weitere Argumente startet Bolt die TUI im aktuellen Verzeichnis.
...
In der Bolt-TUI kannst du `/` plus Befehlsnamen eingeben, um Aktionen schnell auszufuehren.
Zum Beispiel:
...
Fuegt einen Provider zu Bolt hinzu. Erlaubt die Auswahl aus verfuegbaren Providern und das Hinzufuegen ihrer API-Keys.
...
Dies ist getrennt von `bolt.json`, welche das Server-/Runtime-Verhalten konfiguriert.
...
- `leader_timeout` - Steuert, wie lange Bolt nach der leader key wartet. Standard ist `2000`.
...
- `sound_pack` - Zu verwendende sound pack ID. Standard ist `bolt.default`.
