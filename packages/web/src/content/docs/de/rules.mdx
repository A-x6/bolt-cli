...
Wenn du von Claude Code kommst, unterstuetzt Bolt dessen Dateikonventionen als Fallback:
...
Du kannst zusaetzliche Anweisungsdateien in deiner `bolt.json` oder in `~/.config/opencode/bolt.json` angeben.
...
Empfohlen ist das `instructions`-Feld in der `bolt.json`:
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `bolt.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
