---
title: Reglas
description: Establezca instrucciones personalizadas para bolt.
---
...
Para crear un nuevo archivo `AGENTS.md`, puede ejecutar el comando `/init` en bolt.
...
También puedes tener reglas globales en un archivo `~/.config/opencode/AGENTS.md`. Esto se aplica en todas las sesiones bolt.
...
Para los usuarios que migran desde Claude Code, Bolt admite las convenciones de archivos de Claude Code como alternativas:
...
Cuando se inicia bolt, busca archivos de reglas en este orden:
...
Puede especificar archivos de instrucciones personalizados en su `bolt.json` o el `~/.config/opencode/bolt.json` global. Esto le permite a usted y a su equipo reutilizar las reglas existentes en lugar de tener que duplicarlas en AGENTS.md.
...
El enfoque recomendado es utilizar el campo `instructions` en `bolt.json`:
...
:::tip
Para monorepos o proyectos con estándares compartidos, usar `bolt.json` con patrones globales (como `packages/*/AGENTS.md`) es más fácil de mantener que las instrucciones manuales.
:::
