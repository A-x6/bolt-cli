---
title: Configuración
description: Usando la configuración Bolt JSON.
---

Puede configurar Bolt usando un archivo de configuración JSON.

---

## Formato

Bolt admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
Por ejemplo, si su configuración global establece `theme: "bolt"` y `autoupdate: true`, y la configuración de su proyecto establece `model: "anthropic/claude-sonnet-4-5"`, la configuración final incluirá las tres configuraciones.
...
2. **Configuración global** (`~/.config/opencode/bolt.json`) - preferencias del usuario
3. **Configuración personalizada** (`BOLT_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`bolt.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.bolt`** - agentes, comandos, complementos
6. **Configuración en línea** (`BOLT_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.bolt` y `~/.config/opencode` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Coloque su configuración global Bolt en `~/.config/opencode/bolt.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `bolt.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.

:::tip
Coloque la configuración específica del proyecto en la raíz de su proyecto.
:::

Cuando se inicia Bolt, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `BOLT_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `BOLT_CONFIG_DIR`
variable de entorno. En este directorio se buscarán agentes, comandos,
modos y complementos como el directorio estándar `.bolt`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.bolt`, por lo que **puede anular** su configuración.

---

## Esquema

El archivo de configuración tiene un esquema definido en [**`bolt.ai/config.json`**](https://bolt.ai/config.json).
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor Bolt.
- `mdnsDomain` - Nombre de dominio personalizado para el servicio mDNS. El valor predeterminado es `bolt.local`. Útil para ejecutar múltiples instancias en la misma red.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración Bolt a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, Bolt intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración Bolt a través de la opción `theme`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/opencode/agents/` o `.bolt/agents/`. [Más información aquí](/docs/agents).
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, Bolt recurrirá a `"build"` con una advertencia.

Esta configuración se aplica en todas las interfaces: TUI, CLI (`bolt run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/opencode/commands/` o `.bolt/commands/`. [Más información aquí](/docs/commands).
...
Bolt descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
Los [complementos](/docs/plugins) amplían Bolt con herramientas, enlaces e integraciones personalizados.

Coloque los archivos de complemento en `.bolt/plugins/` o `~/.config/opencode/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
...
Esto es útil cuando desea restringir Bolt para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
