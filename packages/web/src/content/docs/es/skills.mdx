...
Las habilidades del agente permiten a Bolt descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Bolt busca estas ubicaciones:

- Configuración del proyecto: `.bolt/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Bolt sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.bolt/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.
...
Crea `.bolt/skills/git-release/SKILL.md` así:
...
Bolt enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `bolt.json`:
...
**Para agentes integrados** (en `bolt.json`):
