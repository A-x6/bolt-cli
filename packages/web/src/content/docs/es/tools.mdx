...
Las herramientas permiten que LLM realice acciones en su código base. Bolt viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Bolt.
...
:::note
Esta herramienta solo está disponible cuando `BOLT_EXPERIMENTAL_LSP_TOOL=true` (o `BOLT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Bolt o cuando la variable de entorno `BOLT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Bolt:

```bash
BOLT_ENABLE_EXA=1 opencode
```
