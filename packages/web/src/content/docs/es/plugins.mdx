---
title: Complementos
description: Escriba sus propios complementos para extender Bolt.
---

Los complementos le permiten extender Bolt al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Bolt.
...
- `.bolt/plugins/` - Complementos a nivel de proyecto
...
1. Configuración global (`~/.config/opencode/bolt.json`)
2. Configuración del proyecto (`bolt.json`)
3. Directorio global de complementos (`~/.config/opencode/plugins/`)
4. Directorio de complementos del proyecto (`.bolt/plugins/`)
...
Bolt ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar bolt.
...
:::note
Si está utilizando la aplicación de escritorio Bolt, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://bolt.ai/docs/sdk) para obtener más detalles.
