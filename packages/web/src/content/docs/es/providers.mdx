---
title: Proveedores
description: Usando cualquier proveedor LLM en Bolt.
---

import config from "../../../../config.mjs"
export const console = config.console

Bolt usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Bolt.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Bolt
configuración.
...
Bolt Zen es una lista de modelos proporcionados por el equipo Bolt que han sido
probado y verificado para funcionar bien con Bolt. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Bolt Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione opencode y diríjase a [bolt.ai/auth](https://bolt.ai/auth).
...
Funciona como cualquier otro proveedor en Bolt y su uso es completamente opcional.

---

## Bolt Go

Bolt Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Bolt que han sido
probado y verificado para funcionar bien con Bolt.

1. Ejecute el comando `/connect` en TUI, seleccione `Bolt Go` y diríjase a [bolt.ai/auth](https://bolt.ai/zen).
...
Para usar Amazon Bedrock con Bolt:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Para una configuración persistente o específica del proyecto, utilice `bolt.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Bolt no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración bolt.
...
para almacenar el token en el almacenamiento de autenticación bolt.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Bolt utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Bolt
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `bolt.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `bolt.json`:
...
Para utilizar Google Vertex AI con Bolt:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta bolt.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de bolt, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación Bolt como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Bolt enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para Bolt. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Bolt:
...
3. Copie la clave API para usarla en Bolt.
...
6. **Importante**: Antes de usar modelos en la nube en Bolt, debe obtener la información del modelo localmente:
...
Bolt Zen es una lista de modelos probados y verificados proporcionada por el equipo Bolt. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Bolt Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Bolt Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Bolt API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración bolt.
...
5. También puedes personalizarlos a través de tu configuración bolt. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Bolt:
...
También puede personalizar modelos a través de su configuración bolt. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con bolt. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `bolt.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Bolt comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
2. Para proveedores personalizados, verifique la configuración opencode y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración bolt.
