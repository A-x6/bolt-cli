...
description: Bolt se integra con sus servidores LSP.
---

Bolt puede integrarse con servidores Language Server Protocol (LSP) para usar diagnósticos como feedback para el agente.

---

## Incorporados

Bolt viene con varios servidores LSP integrados para idiomas populares:
...
:::note
Puede deshabilitar las descargas automáticas del servidor LSP configurando la variable de entorno `BOLT_DISABLE_LSP_DOWNLOAD` en `true`.
:::
...
Puede habilitar y personalizar los servidores LSP a través de la sección `lsp` en su configuración bolt.
