...
透過 Bolt，您可以從多個內建主題中進行選擇，使用能自動適配終端機主題的主題，或者定義您自己的自訂主題。

預設情況下，Bolt 使用我們自己的 `opencode` 主題。
...
Bolt 支援靈活的基於 JSON 的主題系統，讓使用者可以輕鬆建立和自訂主題。
...
3. **專案根目錄** — 定義在 `<project-root>/.bolt/themes/*.json`
4. **當前工作目錄** — 定義在 `./.bolt/themes/*.json`
