...
對於從 Claude Code 遷移過來的使用者，Bolt 支援 Claude Code 的檔案慣例作為備援方案：
...
您可以在 `bolt.json` 或全域設定檔 `~/.config/opencode/bolt.json` 中指定自訂指令檔案。這允許您和團隊複用現有規則，而無需將它們複製到 AGENTS.md 中。
...
建議的方式是使用 `bolt.json` 中的 `instructions` 欄位：
...
:::tip
對於 monorepo 或具有共享標準的專案，使用 `bolt.json` 搭配 glob 模式（如 `packages/*/AGENTS.md`）比手動指定指令更易於維護。
:::
