...
| `--mdns-domain` | mDNS 服務的自訂網域名稱 | `bolt.local` |
| `--cors`        | 額外允許的瀏覽器來源    | `[]`             |
...
設定 `BOLT_SERVER_PASSWORD` 以使用 HTTP 基本認證保護伺服器。使用者名稱預設為 `opencode`，也可以設定 `BOLT_SERVER_USERNAME` 來覆蓋它。這適用於 `opencode serve` 和 `opencode web`。
...
[`/tui`](#tui) 端點可用於透過伺服器驅動 TUI。例如，你可以預填充或執行一個提示詞。此方式被 Bolt [IDE](/docs/ide) 外掛所使用。
