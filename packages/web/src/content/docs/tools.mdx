...
Tools allow the LLM to perform actions in your codebase. Bolt comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Bolt.
...
:::note
This tool is only available when `BOLT_EXPERIMENTAL_LSP_TOOL=true` (or `BOLT_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Bolt or Bolt Go provider, or when either the `BOLT_ENABLE_EXA` or `BOLT_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Bolt:

```bash
BOLT_ENABLE_EXA=1 opencode
# or
BOLT_ENABLE_PARALLEL=1 opencode
```
