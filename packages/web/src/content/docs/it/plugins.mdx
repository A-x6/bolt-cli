---
title: Plugin
description: Scrivi plugin per estendere Bolt.
---

I plugin ti permettono di estendere Bolt agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Bolt.
...
- `.bolt/plugins/` - plugin a livello progetto
...
1. Config globale (`~/.config/opencode/bolt.json`)
2. Config di progetto (`bolt.json`)
3. Directory plugin globale (`~/.config/opencode/plugins/`)
4. Directory plugin di progetto (`.bolt/plugins/`)
...
Bolt esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
Ecco alcuni esempi di plugin che puoi usare per estendere bolt.
...
:::note
Se usi l'app desktop di Bolt, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://bolt.ai/docs/sdk) per i dettagli.
