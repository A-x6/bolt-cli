...
description: Usa la configurazione JSON di Bolt.
---

Puoi configurare Bolt usando un file di configurazione JSON.

---

## Formato

Bolt supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
2. **Config globale** (`~/.config/opencode/bolt.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `BOLT_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`bolt.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.bolt`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `BOLT_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.bolt` e `~/.config/opencode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di Bolt in `~/.config/opencode/bolt.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Aggiungi `bolt.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando Bolt si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `BOLT_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `BOLT_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.bolt` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.bolt`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`bolt.ai/config.json`**](https://bolt.ai/config.json).

La configurazione TUI usa [**`bolt.ai/tui.json`**](https://bolt.ai/tui.json).
...
Usa `BOLT_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `bolt.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Bolt.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `bolt.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in Bolt tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Bolt prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/opencode/agents/` o `.bolt/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Bolt fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`bolt run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/opencode/commands/` o `.bolt/commands/`. [Scopri di piu](/docs/commands).
...
Bolt scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
[I plugin](/docs/plugins) estendono Bolt con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.bolt/plugins/` o `~/.config/opencode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Bolt a usare solo alcuni provider invece di disabilitarli uno per uno.
