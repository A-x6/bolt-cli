---
title: Server LSP
description: Bolt si integra con i tuoi server LSP.
---

Bolt può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Bolt include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `BOLT_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di bolt.
