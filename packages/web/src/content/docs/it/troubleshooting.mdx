...
Per diagnosticare problemi con Bolt, inizia controllando i log e i dati locali che salva su disco.
...
Bolt Desktop esegue in background un server locale di Bolt (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Bolt` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/opencode/bolt.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\bolt.jsonc`
...
Bolt puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Bolt possa ricostruirla.

1. Chiudi completamente Bolt Desktop.
...
3. Riavvia Bolt Desktop.

---

### Risolvi problemi di connessione al server

Bolt Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `bolt.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `BOLT_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `BOLT_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Bolt Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Bolt.

---

### Notifiche non visualizzate

Bolt Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Bolt nelle impostazioni del sistema operativo, e
...
1. Chiudi Bolt Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Bolt Desktop):

- `bolt.settings.dat` (desktop default server URL)
- `bolt.global.dat` e `bolt.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Bolt:
