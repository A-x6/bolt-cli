...
Le skill degli agenti permettono a Bolt di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Bolt cerca in queste posizioni:

- Config di progetto: `.bolt/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Bolt risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.bolt/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.
...
Crea `.bolt/skills/git-release/SKILL.md` cosi':
...
Bolt elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `bolt.json`:
...
**Per agenti integrati** (in `bolt.json`):
