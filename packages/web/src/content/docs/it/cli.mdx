---
title: CLI
description: Opzioni e comandi della CLI di Bolt.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di Bolt, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
opencode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con Bolt in modo programmatico.
...
Avvia l'interfaccia testuale di Bolt (interfaccia utente da terminale).
...
La CLI di Bolt include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per Bolt.
...
Collega un terminale a un backend server di Bolt già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
opencode attach [url]
```

Questo consente di usare la TUI con un backend Bolt remoto. Per esempio:
...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `BOLT_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `BOLT_SERVER_USERNAME` o `opencode`) |
...
Bolt si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `opencode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Quando Bolt si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in Bolt.
...
Avvia un server Bolt headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
opencode serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `BOLT_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Mostra statistiche di utilizzo token e costo per le sessioni Bolt.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione Bolt.

```bash
opencode import <file>
```

Puoi importare da un file locale o da un URL di condivisione Bolt.
...
Avvia un server Bolt headless con interfaccia web.

```bash
opencode web
```

Avvia un server HTTP e apre un browser per accedere a Bolt tramite interfaccia web. Imposta `BOLT_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Disinstalla Bolt e rimuove tutti i file correlati.
...
Bolt può essere configurato tramite variabili d'ambiente.

| Variabile                             | Tipo    | Descrizione                                                 |
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `BOLT_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `BOLT_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `BOLT_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `BOLT_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
| `BOLT_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `BOLT_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `BOLT_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `BOLT_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `BOLT_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `BOLT_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `BOLT_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `BOLT_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `BOLT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `BOLT_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `BOLT_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `BOLT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `BOLT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `BOLT_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `BOLT_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `BOLT_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `BOLT_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `BOLT_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `BOLT_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
| `BOLT_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                       | Tipo    | Descrizione                                             |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `BOLT_EXPERIMENTAL`                         | boolean | Abilita funzionalità sperimentali sotto il flag globale |
| `BOLT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                                  |
| `BOLT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI                     |
| `BOLT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms               |
| `BOLT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM                |
| `BOLT_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir                   |
| `BOLT_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                                 |
| `BOLT_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale                      |
| `BOLT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                                 |
| `BOLT_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalità Exa sperimentali                   |
| `BOLT_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python                        |
| `BOLT_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                                       |
| `BOLT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Abilita task subagent in background                     |
| `BOLT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Abilita sistema eventi sperimentale                     |
| `BOLT_EXPERIMENTAL_NATIVE_LLM`              | boolean | Abilita percorso nativo di richiesta LLM                |
| `BOLT_EXPERIMENTAL_PARALLEL`                | boolean | Abilita esecuzione parallela della ricerca web          |
| `BOLT_EXPERIMENTAL_SCOUT`                   | boolean | Abilita subagent Scout                                  |
| `BOLT_EXPERIMENTAL_WORKSPACES`              | boolean | Abilita supporto workspace                              |
