---
title: Provider
description: Usa qualsiasi provider LLM in Bolt.
---

import config from "../../../../config.mjs"
export const console = config.console

Bolt usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Bolt.
...
Bolt Zen e una lista di modelli forniti dal team Bolt, testati e
verificati per funzionare bene con Bolt. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Bolt Zen.
:::

1. Esegui `/connect` nella TUI, seleziona opencode e vai su [bolt.ai/auth](https://bolt.ai/auth).
...
Funziona come qualsiasi altro provider in Bolt ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Bolt:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `bolt.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Bolt non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di bolt.
...
salvare il token nello storage di auth di bolt.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Bolt usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Bolt
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `bolt.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `bolt.json`:
...
Per usare Google Vertex AI con Bolt:
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da bolt, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di Bolt come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Bolt elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per Bolt. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con Bolt:
...
3. Copia la chiave API da usare in Bolt.
...
6. **Importante**: Prima di usare modelli cloud in Bolt, devi scaricare le informazioni del modello localmente:
...
Bolt Zen è una lista di modelli testati e verificati forniti dal team Bolt. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Bolt Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Bolt Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Bolt.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di bolt.
...
5. Puoi anche personalizzarli tramite la configurazione di bolt. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di bolt. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con bolt. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `bolt.json` nella directory del progetto:
...
I campi `limit` permettono a Bolt di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
2. Per provider personalizzati, controlla la config di opencode e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config bolt.
