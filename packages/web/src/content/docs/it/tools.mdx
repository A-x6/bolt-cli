...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Bolt include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Bolt.
...
:::note
Questo strumento e' disponibile solo quando `BOLT_EXPERIMENTAL_LSP_TOOL=true` (oppure `BOLT_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Bolt o quando la variabile d'ambiente `BOLT_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Bolt:
