---
title: Regole
description: Imposta istruzioni personalizzate per bolt.
---
...
Per creare un nuovo file `AGENTS.md`, puoi eseguire il comando `/init` in bolt.
...
Puoi anche avere regole globali in un file `~/.config/opencode/AGENTS.md`. Queste si applicano a tutte le sessioni di bolt.
...
Per gli utenti che migrano da Claude Code, Bolt supporta come fallback le convenzioni di file di Claude Code:
...
Puoi specificare file di istruzioni personalizzati nel tuo `bolt.json` o nel globale `~/.config/opencode/bolt.json`. Questo permette a te e al tuo team di riusare regole esistenti invece di doverle duplicare in AGENTS.md.
...
L'approccio consigliato e' usare il campo `instructions` in `bolt.json`:
...
:::tip
Per monorepo o progetti con standard condivisi, usare `bolt.json` con pattern glob (come `packages/*/AGENTS.md`) e' piu' manutenibile rispetto alle istruzioni manuali.
:::
