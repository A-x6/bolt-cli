...
| <nobr><code>{"--password"}</code></nobr> | `-p` | 기본 인증 비밀번호 (기본값: `BOLT_SERVER_PASSWORD`)                    |
| <nobr><code>{"--username"}</code></nobr> | `-u` | 기본 인증 사용자 이름 (기본값: `BOLT_SERVER_USERNAME` 또는 `opencode`) |
...
Bolt는 [Models.dev](https://models.dev)의 provider 목록을 기반으로 동작하므로, `opencode auth login`으로 원하는 provider의 API 키를 설정할 수 있습니다. 인증 정보는 `~/.local/share/opencode/auth.json`에 저장됩니다.

```bash
opencode auth login
```

Bolt 시작 시 인증 파일에서 provider 정보를 불러오며, 시스템 환경 변수나 프로젝트의 `.env`에 정의된 키도 함께 로드합니다.
...
`--refresh` 플래그를 사용하면 캐시된 모델 목록을 갱신할 수 있습니다. provider에 새 모델이 추가된 뒤 Bolt에서 바로 확인하고 싶을 때 유용합니다.
...
API 접근용 headless Bolt 서버를 시작합니다. 전체 HTTP 인터페이스는 [server docs](/docs/server)를 참고하세요.

```bash
opencode serve
```

이 명령은 TUI 없이 opencode 기능에 접근할 수 있는 HTTP 서버를 시작합니다. `BOLT_SERVER_PASSWORD`를 설정하면 HTTP basic auth가 활성화됩니다(기본 사용자명: `opencode`).
...
JSON 파일 또는 Bolt 공유 URL에서 세션 데이터를 가져옵니다.

```bash
opencode import <file>
```

로컬 파일이나 Bolt 공유 URL에서 가져올 수 있습니다.
...
웹 인터페이스를 포함한 headless Bolt 서버를 시작합니다.

```bash
opencode web
```

이 명령은 HTTP 서버를 시작하고 웹 브라우저를 열어 웹 인터페이스로 Bolt에 접속합니다. `BOLT_SERVER_PASSWORD`를 설정하면 HTTP basic auth가 활성화됩니다(기본 사용자명: `opencode`).
...
Bolt는 환경 변수로도 구성할 수 있습니다.

| 변수                                  | 타입    | 설명                                           |
| ------------------------------------- | ------- | ---------------------------------------------- |
| `BOLT_AUTO_SHARE`                 | boolean | 세션 자동 공유                                 |
| `BOLT_GIT_BASH_PATH`              | string  | Windows에서 Git Bash 실행 파일 경로            |
| `BOLT_CONFIG`                     | string  | 설정 파일 경로                                 |
| `BOLT_CONFIG_DIR`                 | string  | 설정 디렉터리 경로                             |
| `BOLT_CONFIG_CONTENT`             | string  | 인라인 JSON 설정 내용                          |
| `BOLT_DISABLE_AUTOUPDATE`         | boolean | 자동 업데이트 확인 비활성화                    |
| `BOLT_DISABLE_PRUNE`              | boolean | 오래된 데이터 정리(prune) 비활성화             |
| `BOLT_DISABLE_TERMINAL_TITLE`     | boolean | 터미널 제목 자동 업데이트 비활성화             |
| `BOLT_PERMISSION`                 | string  | 인라인 JSON 권한 설정                          |
| `BOLT_DISABLE_DEFAULT_PLUGINS`    | boolean | 기본 플러그인 비활성화                         |
| `BOLT_DISABLE_LSP_DOWNLOAD`       | boolean | LSP 서버 자동 다운로드 비활성화                |
| `BOLT_ENABLE_EXPERIMENTAL_MODELS` | boolean | 실험적 모델 활성화                             |
| `BOLT_DISABLE_AUTOCOMPACT`        | boolean | 자동 컨텍스트 컴팩션 비활성화                  |
| `BOLT_DISABLE_CLAUDE_CODE`        | boolean | `.claude`(프롬프트 + 스킬) 읽기 비활성화       |
| `BOLT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | `~/.claude/CLAUDE.md` 읽기 비활성화            |
| `BOLT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | `.claude/skills` 로드 비활성화                 |
| `BOLT_DISABLE_MODELS_FETCH`       | boolean | 원격 소스에서 모델 목록 가져오기 비활성화      |
| `BOLT_FAKE_VCS`                   | string  | 테스트용 가짜 VCS provider                     |
| `BOLT_CLIENT`                     | string  | 클라이언트 식별자(기본값: `cli`)               |
| `BOLT_ENABLE_EXA`                 | boolean | Exa 웹 검색 도구 활성화                        |
| `BOLT_SERVER_PASSWORD`            | string  | `serve`/`web` 기본 인증 활성화                 |
| `BOLT_SERVER_USERNAME`            | string  | 기본 인증 사용자명 오버라이드(기본 `opencode`) |
| `BOLT_MODELS_URL`                 | string  | 모델 설정을 가져올 사용자 지정 URL             |
...
| 변수                                            | 타입    | 설명                                        |
| ----------------------------------------------- | ------- | ------------------------------------------- |
| `BOLT_EXPERIMENTAL`                         | boolean | 엄브렐라 플래그로 제어되는 실험 기능 활성화 |
| `BOLT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | 아이콘 탐색 활성화                          |
| `BOLT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | TUI에서 선택 시 복사 비활성화               |
| `BOLT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | bash 명령 기본 타임아웃(ms)                 |
| `BOLT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | LLM 응답 최대 출력 토큰 수                  |
| `BOLT_EXPERIMENTAL_FILEWATCHER`             | boolean | 전체 디렉터리 파일 감시 활성화              |
| `BOLT_EXPERIMENTAL_OXFMT`                   | boolean | oxfmt 포매터 활성화                         |
| `BOLT_EXPERIMENTAL_LSP_TOOL`                | boolean | 실험적 LSP 도구 활성화                      |
| `BOLT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | 파일 감시 비활성화                          |
| `BOLT_EXPERIMENTAL_EXA`                     | boolean | 실험적 Exa 기능 활성화                      |
| `BOLT_EXPERIMENTAL_LSP_TY`                  | boolean | python 파일에 대해 TY LSP 활성화            |
| `BOLT_EXPERIMENTAL_PLAN_MODE`               | boolean | Plan mode 활성화                            |
| `BOLT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | 백그라운드 subagent 작업 활성화             |
| `BOLT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | 실험적 이벤트 시스템 활성화                 |
| `BOLT_EXPERIMENTAL_NATIVE_LLM`              | boolean | 네이티브 LLM 요청 경로 활성화               |
| `BOLT_EXPERIMENTAL_PARALLEL`                | boolean | 병렬 웹 검색 실행 활성화                    |
| `BOLT_EXPERIMENTAL_SCOUT`                   | boolean | Scout subagent 활성화                       |
| `BOLT_EXPERIMENTAL_WORKSPACES`              | boolean | workspace 지원 활성화                       |
