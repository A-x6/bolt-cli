...
- macOS만: `Bolt` 메뉴 -> **웹뷰 **(UI가 공백/frozen인 경우).
...
-**macOS/리눅스**: `~/.config/opencode/bolt.jsonc` (또는 `~/.config/opencode/bolt.json`) -**macOS/Linux** (외부 설치): `~/.local/share/opencode/bolt.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\opencode\bolt.jsonc`
...
`bolt.json(c)`가 `server` 섹션을 포함하면 일시적으로 제거하고 데스크톱 앱을 다시 시작합니다.

### 환경 변수 확인

`BOLT_PORT`가 있는 경우, 데스크탑 앱은 로컬 서버의 포트를 사용하려고 합니다.

- `BOLT_PORT` (또는 무료 포트를 선택) 및 재시작.
...
- `bolt.settings.dat` (데스크톱 기본 서버 URL)
- `bolt.global.dat` 및 `bolt.workspace.*.dat` (최근 서버/프로젝트와 같은 UI 국가)
