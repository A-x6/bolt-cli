---
title: IDE
description: VS Code, Cursor 및 기타 IDE용 Bolt 확장 프로그램
---

Bolt는 VS Code, Cursor, 또는 터미널을 지원하는 모든 IDE와 통합됩니다. 시작하려면 터미널에서 `opencode`를 실행하세요.

---

## 사용법

- **Quick Launch**: `Cmd+Esc` (Mac) 또는 `Ctrl+Esc` (Windows/Linux)를 사용해 분할 터미널 뷰에서 Bolt를 열거나, 이미 실행 중인 터미널 세션으로 포커스하세요.
- **New Session**: `Cmd+Shift+Esc` (Mac) 또는 `Ctrl+Shift+Esc` (Windows/Linux)를 사용해 기존 세션이 열려 있어도 새 Bolt 터미널 세션을 시작하세요. UI의 Bolt 버튼을 클릭해도 됩니다.
- **Context Awareness**: 현재 선택 영역이나 탭을 Bolt와 자동으로 공유합니다.
...
VS Code와 Cursor, Windsurf, VSCodium 같은 인기 포크에 Bolt를 설치하려면:
...
Extension Marketplace에서 **Bolt**를 검색한 다음 **Install**을 클릭하세요.
