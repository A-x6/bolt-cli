...
:::caution
`BOLT_SERVER_PASSWORD`가 설정되지 않은 경우 서버가 중단되지 않습니다. 이것은 로컬 사용을위한 괜찮지만 네트워크 액세스를 설정해야합니다.
:::
...
이 자동으로 `0.0.0.0`에 hostname을 설정하고 `bolt.local`로 서버를 광고합니다.
...
액세스를 보호하려면 `BOLT_SERVER_PASSWORD` 환경 변수를 사용하여 암호를 설정하십시오.

```bash
BOLT_SERVER_PASSWORD=secret opencode web
```

`opencode`에 대한 사용자의 기본값은 `BOLT_SERVER_USERNAME`로 변경할 수 있습니다.
...
`bolt.json` config 파일에서 서버 설정을 구성할 수 있습니다:
