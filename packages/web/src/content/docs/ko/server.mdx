---
title: 서버
description: HTTP를 통해 Bolt 서버와 상호 작용합니다.
---
...
| `--mdns-domain` | mDNS 서비스의 사용자 지정 도메인 이름 | `bolt.local` |
...
`BOLT_SERVER_PASSWORD`를 설정하여 서버를 HTTP Basic auth로 보호합니다. `opencode` 또는 `BOLT_SERVER_USERNAME`를 오버라이드로 설정하는 사용자의 기본값. 이것은 `opencode serve`와 `opencode web` 둘 다에 적용합니다.
