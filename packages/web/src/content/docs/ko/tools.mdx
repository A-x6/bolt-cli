...
:::note
이 도구는 `BOLT_EXPERIMENTAL_LSP_TOOL=true` (또는 `BOLT_EXPERIMENTAL=true`) 때만 유효합니다.
:::
...
:::note
이 도구는 opencode 제공 업체 또는 `BOLT_ENABLE_EXA` 환경 변수가 모든 진실 값 (예 : `true` 또는 `1`)로 설정할 때만 사용할 수 있습니다.
