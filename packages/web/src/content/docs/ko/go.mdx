...
Bolt Go는 인기 있는 오픈 코딩 모델에 안정적으로 액세스할 수 있게 해주는 **월 $10**의 저비용 구독 서비스입니다.

Go는 Bolt의 다른 제공자와 똑같이 작동합니다. Bolt Go를 구독하고 API 키를 발급받으면 됩니다. 이는 **완전히 선택 사항**이며, Bolt를 사용하기 위해 꼭 필요하지는 않습니다.
...
:::tip
Bolt와 잘 맞는 선별된 모델과 제공자 그룹을 테스트했습니다.
:::
...
Bolt Go를 사용하면 **월 $10**으로 이러한 모델에 액세스할 수 있습니다.

---

## 작동 방식

Bolt Go는 Bolt의 다른 제공자와 똑같이 작동합니다.

1. **<a href={console}>Bolt Zen</a>**에 로그인해 Go를 구독하고 API 키를 복사합니다.
2. TUI에서 `/connect` 명령을 실행하고 `Bolt Go`를 선택한 다음 API 키를 붙여넣습니다.
3. TUI에서 `/models`를 실행해 Go를 통해 사용할 수 있는 모델 목록을 확인합니다.

:::note
workspace당 한 명의 멤버만 Bolt Go를 구독할 수 있습니다.
:::
...
Bolt Go는 [Bolt](https://bolt.ai) 및 유사한 유형의 요청을 생성하는 다른 코딩 에이전트용으로 설계되었습니다.
...
Bolt 외에도 다음 클라이언트가 Bolt Go에서 올바르게 작동하는 것으로 검증되었습니다.
다만 앞으로도 계속 작동할 것이라고 보장하지는 않습니다.

| 클라이언트        | 세션 지원                                                                                                                                                                                                                                                    |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Hermes**        | [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864)가 포함된 빌드는 Bolt의 기본 요청과 보조 요청 모두에서 헤더를 전송합니다. 이 수정은 v0.21.0 이후에 병합되었으므로 v0.21.0 릴리스 자체에는 포함되지 않습니다.                       |
...
| **Pi**            | 현재 빌드는 Bolt에 세션 정보를 전송합니다. 이전 설치 버전은 업데이트하세요.                                                                                                                                                                              |
| **jcode**         | [세션 헤더 수정](https://github.com/1jehuang/jcode/issues/1167)이 포함된 **v0.81.6 이상**으로 업데이트하세요.                                                                                                                                                |
| **Kilo Code CLI** | [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752)가 포함된 빌드는 Bolt 세션 헤더를 복원합니다. 이 수정은 CLI에만 적용되며 VS Code 확장에는 적용되지 않습니다. [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723)을 참조하세요. |
...
Bolt config의 [model id](/docs/config/#models)는 `opencode-go/<model-id>` 형식을 사용합니다. 예를 들어 Kimi K3의 경우 config에서 `opencode-go/kimi-k3`를 사용하면 됩니다.
...
4. Bolt에서 다른 제공자도 함께 사용할 수 있게 해 **종속성 없음**을 유지합니다.
