---
title: 규칙
description: Bolt에 대한 사용자 정의 지침 설정.
---
...
`bolt.json` 또는 글로벌 `~/.config/opencode/bolt.json`에서 사용자 지정 명령 파일을 지정할 수 있습니다. AGENTS.md에 복제하지 않고 기존 규칙을 재사용 할 수 있습니다.
...
`instructions`의 `bolt.json` 필드를 사용하는 것이 좋습니다.
...
:::tip
공유 표준을 가진 monorepos 또는 프로젝트를 위해, glob 본을 가진 `bolt.json`를 사용하여 (`packages/*/AGENTS.md` 같이)는 수동 지시 보다는 더 유지가능합니다.
:::
