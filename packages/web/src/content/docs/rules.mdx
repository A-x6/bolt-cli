...
To create a new `AGENTS.md` file, you can run the `/init` command in bolt.
...
For users migrating from Claude Code, Bolt supports Claude Code's file conventions as fallbacks:
...
You can specify custom instruction files in your `bolt.json` or the global `~/.config/opencode/bolt.json`. This allows you and your team to reuse existing rules rather than having to duplicate them to AGENTS.md.
...
The recommended approach is to use the `instructions` field in `bolt.json`:
...
:::tip
For monorepos or projects with shared standards, using `bolt.json` with glob patterns (like `packages/*/AGENTS.md`) is more maintainable than manual instructions.
:::
