...
Bolt Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli **aylık 10$ aboneliğidir**.

Go, Bolt'daki diğer sağlayıcılar gibi çalışır. Bolt Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Bolt'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Bolt Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Bolt Go, Bolt'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Bolt Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Bolt Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Bolt Go'ya abone olabilir.
:::
...
Bolt Go, [Bolt](https://bolt.ai) ve benzer türde istekler üreten diğer kodlama aracıları için tasarlanmıştır.
...
Bolt'un yanı sıra aşağıdaki istemcilerin Bolt Go ile düzgün çalıştığı doğrulanmıştır.
Ancak gelecekte de çalışmaya devam edeceklerini garanti etmiyoruz.

| İstemci           | Oturum desteği                                                                                                                                                                                                                                               |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Hermes**        | [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) içeren derlemeler, ana ve yardımcı Bolt isteklerinde başlığı gönderir. Düzeltme v0.21.0'dan sonra birleştirilmiştir; dolayısıyla v0.21.0 sürümü tek başına bu düzeltmeyi içermez. |
...
| **Pi**            | Güncel derlemeler Bolt için oturum bilgilerini gönderir. Eski kurulumları güncelleyin.                                                                                                                                                                   |
...
| **Kilo Code CLI** | [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) içeren derlemeler Bolt oturum başlıklarını geri getirir. Bu düzeltme CLI'ı kapsar, VS Code uzantısını kapsamaz. Bkz. [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723).     |
...
Bolt yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `opencode-go/kimi-k3` kullanmalısınız.
...
Bolt Go'yu şu amaçlarla oluşturduk:
...
4. Bolt ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
