...
- Proje konfigurasyonu: `.bolt/skills/<name>/SKILL.md`
...
Proje yerel yollarinda bolt, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.bolt/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.
...
`.bolt/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
bolt, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`bolt.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`bolt.json` icinde):
