...
bolt, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
Bolt Zen, opencode ekibi tarafından sağlanan modellerin bir listesidir.
opencode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Bolt Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, opencode'u seçin ve [bolt.ai/auth](https://bolt.ai/auth)'ye gidin.
...
Bolt Go, opencode ile iyi çalıştığı test edilmiş ve doğrulanmış, opencode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Bolt Go`'yu seçin ve [bolt.ai/auth](https://bolt.ai/zen) adresine gidin.
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `bolt.json` kullanın:
...
:::note[uyumluluk notu]
bolt, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
...
`bolt.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`bolt.json` üzerinden özelleştirin:
...
Bolt Zen, opencode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Bolt Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Bolt Zen**'i arayın.
...
4. Proje dizininizde `bolt.json` dosyanızı oluşturun veya güncelleyin:
