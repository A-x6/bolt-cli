...
Örneğin, genel ayarınız `theme: "bolt"` ve `autoupdate: true`'yi ayarlıyorsa ve proje ayarlarınız `model: "anthropic/claude-sonnet-4-5"`'yi ayarlıyorsa, nihai yapılandırma üç ayarı da içerecektir.
...
2. **Genel yapılandırma** (`~/.config/opencode/bolt.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`BOLT_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `bolt.json`) - projeye özgü ayarlar
5. **`.bolt` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`BOLT_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.bolt` ve `~/.config/opencode` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Global opencode yapılandırmanızı `~/.config/opencode/bolt.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `bolt.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.
...
`BOLT_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`BOLT_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.bolt` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.bolt` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.

---

## Şema

Yapılandırma dosyası [**`bolt.ai/config.json`**](https://bolt.ai/config.json)'da tanımlanan bir şemaya sahiptir.
...
- `mdnsDomain` - mDNS hizmeti için özel alan adı. Varsayılan olarak `bolt.local` şeklindedir. Aynı ağda birden fazla örneği çalıştırmak için kullanışlıdır.
...
`small_model` seçeneği, başlık oluşturma gibi hafif görevler için ayrı bir model yapılandırır. Varsayılan olarak bolt, sağlayıcınızda mevcutsa daha ucuz bir model kullanmaya çalışır, aksi takdirde ana modelinize geri döner.
...
Agent'ları `~/.config/opencode/agents/` veya `.bolt/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`bolt run`), masaüstü uygulaması ve GitHub Action.
...
Komutları `~/.config/opencode/commands/` veya `.bolt/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
Eklenti dosyalarını `.bolt/plugins/` veya `~/.config/opencode/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
