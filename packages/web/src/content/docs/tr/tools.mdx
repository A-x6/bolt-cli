...
:::note
Bu araç yalnızca `BOLT_EXPERIMENTAL_LSP_TOOL=true` (veya `BOLT_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca opencode provider kullanılırken veya `BOLT_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.
