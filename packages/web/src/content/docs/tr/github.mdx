...
bolt, GitHub iş akışınızla bütünleşir. Yorumunuzda `/opencode` veya `/oc`'den bahsedin; bolt, GitHub Actions çalıştırıcınızdaki görevleri yürütecektir.
...
- **Güvenli**: bolt, GitHub'ınızın çalıştırıcılarının içinde çalışır.
...
2. **İş akışını ekleyin**
   ...
   Aşağıdaki iş akışı dosyasını deponuzdaki `.github/workflows/bolt.yml`'a ekleyin. `model`'de uygun `env` ve gerekli API anahtarlarını ayarladığınızdan emin olun.
...
- `token`: Yorum oluşturma, değişiklik yapma ve çekme isteklerini açma gibi işlemleri gerçekleştirmek için isteğe bağlı GitHub erişim belirteci. Varsayılan olarak bolt, opencode GitHub Uygulamasındaki kurulum erişim belirtecini kullanır, bu nedenle taahhütler, yorumlar ve çekme istekleri uygulamadan geliyormuş gibi görünür.
...
- **Bir sorunu açıklama**
  ...
  bolt, tüm yorumlar da dahil olmak üzere ileti dizisinin tamamını okuyacak ve net bir açıklama ile yanıt verecektir.
...
- **Belirli kod satırlarını inceleme**
  ...
  PR'nin "Dosyalar" sekmesindeki kod satırlarına doğrudan yorum bırakın. bolt, kesin yanıtlar sağlamak için dosyayı, satır numaralarını ve fark içeriğini otomatik olarak algılar.
