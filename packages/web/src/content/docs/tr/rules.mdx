...
bolt, Claude Code'dan geçiş yapan kullanıcılar için yedek olarak Claude Code'un dosya kurallarını destekler:
...
Özel talimat dosyalarını `bolt.json` veya global `~/.config/opencode/bolt.json`'de belirtebilirsiniz. Bu, sizin ve ekibinizin, mevcut kuralları AGENTS.md'ye kopyalamak yerine yeniden kullanmasına olanak tanır.
...
### bolt.json'u kullanma

Önerilen yaklaşım, `instructions`'deki `bolt.json` alanını kullanmaktır:
...
:::tip
Monorepo'lar veya ortak standartlara sahip projeler için, `bolt.json`'yi glob desenleriyle (`packages/*/AGENTS.md` gibi) kullanmak, manuel talimatlara göre daha sürdürülebilirdir.
:::
