...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `bolt.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `BOLT_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `opencode` değeridir; değiştirmek isterseniz `BOLT_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `opencode serve` hem de `opencode web` için geçerlidir.
