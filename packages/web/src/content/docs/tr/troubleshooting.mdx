...
bolt, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:
...
- **macOS/Linux**: `~/.config/opencode/bolt.jsonc` (veya `~/.config/opencode/bolt.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/opencode/bolt.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\bolt.jsonc` yapistirin
...
`bolt.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `BOLT_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `BOLT_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
- `bolt.settings.dat` (desktop varsayilan sunucu URL'si)
- `bolt.global.dat` ve `bolt.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. bolt, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
bolt, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
