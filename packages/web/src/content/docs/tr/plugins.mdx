---
title: Eklentiler
description: Bolt'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Bolt'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Bolt'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.bolt/plugins/` - Proje düzeyinde eklentiler
...
1. Global config (`~/.config/opencode/bolt.json`)
2. Project config (`bolt.json`)
3. Global eklenti dizini (`~/.config/opencode/plugins/`)
4. Proje eklenti dizini (`.bolt/plugins/`)
...
Bolt bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Bolt SDK istemcisi.
...
Bolt'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Bolt masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Bolt'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Bolt'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Bolt'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Bolt'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://bolt.ai/docs/sdk) bakın.
