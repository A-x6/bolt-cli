...
通过 Bolt，您可以从多个内置主题中进行选择，使用能自动适配终端主题的主题，或者定义您自己的自定义主题。

默认情况下，Bolt 使用我们自己的 `opencode` 主题。
...
Bolt 支持灵活的基于 JSON 的主题系统，让用户可以轻松创建和自定义主题。
...
3. **项目根目录** — 定义在 `<project-root>/.bolt/themes/*.json`
4. **当前工作目录** — 定义在 `./.bolt/themes/*.json`
