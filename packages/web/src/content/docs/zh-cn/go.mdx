...
Bolt Go 是一项**每月 10 美元的低成本订阅服务**，让你能够稳定地访问流行的开源编程模型。

Go 的工作方式与 Bolt 中的任何其他提供商（provider）一样。订阅 Bolt Go 后你将获得 API 密钥。它是 **完全可选** 的，并非使用 Bolt 所必需的条件。
...
:::tip
我们测试了一组经过精选且与 Bolt 配合良好的模型和提供商。
:::
...
Bolt Go 让你能够以**每月 10 美元**的价格访问这些模型。

---

## 工作原理

Bolt Go 的工作方式与 Bolt 中的其他提供商一样。

1. 登录 **<a href={console}>Bolt Zen</a>**，订阅 Go，然后复制你的 API 密钥。
2. 在 TUI 中运行 `/connect` 命令，选择 `Bolt Go`，然后粘贴你的 API 密钥。
3. 在 TUI 中运行 `/models` 以查看通过 Go 可用的模型列表。

:::note
每个工作空间只能有一名成员订阅 Bolt Go。
:::
...
Bolt Go 适用于 [Bolt](https://bolt.ai) 以及其他会产生类似请求的编程 Agent。
...
除 Bolt 外，以下客户端已通过验证，能够正常使用 Bolt Go。但我们无法保证它们未来仍能正常使用。

| 客户端            | 会话支持                                                                                                                                                                                                                    |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | 包含 [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) 的构建版本会在主要和辅助 Bolt 请求中发送该请求头。此修复在 v0.21.0 发布后才合并，因此 v0.21.0 本身并不包含该修复。                          |
...
| **Pi**            | 当前构建版本会为 Bolt 发送会话信息。请更新旧版安装。                                                                                                                                                                    |
| **jcode**         | 请更新至 **v0.81.6 或更高版本**，其中包含[会话请求头修复](https://github.com/1jehuang/jcode/issues/1167)。                                                                                                                  |
| **Kilo Code CLI** | 包含 [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) 的构建版本恢复了 Bolt 会话请求头。此修复仅适用于 CLI，不适用于 VS Code 扩展。参见 [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723)。 |
...
你的 Bolt 配置中的 [模型 ID](/docs/config/#models) 使用 `opencode-go/<model-id>` 格式。例如，对于 Kimi K3，你将在配置中使用 `opencode-go/kimi-k3`。
...
4. **无锁定（no lock-in）**，允许你与 Bolt 一起使用任何其他提供商。
