...
代理技能让 Bolt 能够从你的仓库或主目录中发现可复用的指令。
...
- 项目配置：`.bolt/skills/<name>/SKILL.md`
...
对于项目本地路径，Bolt 会从当前工作目录向上遍历，直到到达 git 工作树根目录。
在此过程中，它会加载 `.bolt/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。
...
创建 `.bolt/skills/git-release/SKILL.md`，内容如下：
...
在 `bolt.json` 中使用基于模式的权限来控制代理可以访问哪些技能：
