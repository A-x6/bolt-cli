...
对于从 Claude Code 迁移过来的用户，Bolt 支持 Claude Code 的文件约定作为回退方案：
...
您可以在 `bolt.json` 或全局配置文件 `~/.config/opencode/bolt.json` 中指定自定义指令文件。这允许您和团队复用现有规则，而无需将它们复制到 AGENTS.md 中。
...
推荐的方式是使用 `bolt.json` 中的 `instructions` 字段：
...
:::tip
对于 monorepo 或具有共享标准的项目，使用 `bolt.json` 配合 glob 模式（如 `packages/*/AGENTS.md`）比手动指定指令更易于维护。
:::
