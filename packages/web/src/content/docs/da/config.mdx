---
title: Konfiguration
description: Ved at bruge Bolt JSON konfig.
---

Du kan konfigurere Bolt ved hjælp af en JSON-konfigurationsfil.

---

## Format

Bolt understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global config** (`~/.config/opencode/bolt.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`BOLT_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`bolt.json` i projekt) - projektspecifikke indstillinger
5. **`.bolt` mapper** - agent, kommandoer, plugins
6. **Inline config** (`BOLT_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.bolt` og `~/.config/opencode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Placer din globale Bolt-konfiguration i `~/.config/opencode/bolt.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `bolt.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Bolt starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `BOLT_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `BOLT_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.bolt` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.bolt` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`bolt.ai/config.json`**](https://bolt.ai/config.json).
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Bolt-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `bolt.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Bolt-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Bolt at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Bolt-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/opencode/agents/` eller `.bolt/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Bolt falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`bolt run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/opencode/commands/` eller `.bolt/commands/`. [Learn more here](/docs/commands).
...
Bolt vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
[Plugins](/docs/plugins) udvide Bolt med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.bolt/plugins/` eller `~/.config/opencode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Bolt til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
