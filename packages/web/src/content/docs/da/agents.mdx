...
Der er to typer agenter i Bolt; primære agenter og subagenter.
...
Bolt leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Bolt leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

Bolt leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i Bolt's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `bolt.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger Bolt modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Bolt-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Bolt-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [Bolt Zen](/docs/zen), vil du bruge `opencode/gpt-5.1-codex` til GPT 5.1 Codex.
