---
title: CLI
description: Bolt CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Bolt CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
opencode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med Bolt programmatisk.
...
Start Bolt terminalbrugergrænsefladen.
...
Tilslut en terminal til en allerede kørende Bolt backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern Bolt backend. For eksempel:
...
| <nobr><code>{"--session"}</code></nobr>  | `-s` | Sessions-id for at fortsætte                                                      |
| <nobr><code>{"--fork"}</code></nobr>     |      | Forgren sessionen ved fortsættelse (brug med `--continue` eller `--session`)      |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `BOLT_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `BOLT_SERVER_USERNAME` eller `opencode`) |
...
Bolt drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `opencode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Når Bolt starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i Bolt.
...
Start en hovedløs Bolt-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
opencode serve
```

Dette starter en HTTP-server, der giver API-adgang til opencode-funktionalitet uden TUI-grænsefladen. Indstil `BOLT_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Administrator Bolt sessionsholder.
...
Liste over alle Bolt sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine Bolt-sessioner.
...
Importer sessionsdata fra en JSON fil eller Bolt del URL.
...
Start en hovedløs Bolt-server med en webgrænseflade.

```bash
opencode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til Bolt via en webgrænseflade. Indstil `BOLT_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Afinstaller Bolt og fjern alle relaterede filer.
...
Opdaterer Bolt til den nyeste version eller en specifik version.
...
Bolt kan konfigureres ved hjælp af miljøvariabler.

| Variabel                              | Skriv   | Beskrivelse                                                           |
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `BOLT_AUTO_SHARE`                 | boolean | Del automatisk session                                                |
| `BOLT_GIT_BASH_PATH`              | string  | Sti til Git Bash eksekverbar på Windows                               |
| `BOLT_CONFIG`                     | string  | Sti til konfigurationsfil                                             |
| `BOLT_TUI_CONFIG`                 | string  | Sti til TUI-konfigurationsfil                                         |
| `BOLT_CONFIG_DIR`                 | string  | Sti til konfigurationsmappe                                           |
| `BOLT_CONFIG_CONTENT`             | string  | Indbygget json-konfigurationsindhold                                  |
| `BOLT_DISABLE_AUTOUPDATE`         | boolean | Deaktiver automatisk opdateringskontrol                               |
| `BOLT_DISABLE_PRUNE`              | boolean | Deaktiver beskæring af gamle data                                     |
| `BOLT_DISABLE_TERMINAL_TITLE`     | boolean | Deaktiver automatisk opdatering af terminaltitel                      |
| `BOLT_PERMISSION`                 | string  | Indbygget json-tilladelseskonfiguration                               |
| `BOLT_DISABLE_DEFAULT_PLUGINS`    | boolean | Deaktiver standard plugins                                            |
| `BOLT_DISABLE_LSP_DOWNLOAD`       | boolean | Deaktiver automatisk LSP-serverdownloads                              |
| `BOLT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Aktive eksperimentelle modeller                                       |
| `BOLT_DISABLE_AUTOCOMPACT`        | boolean | Deaktiver automatisk kontekstkomprimering                             |
| `BOLT_DISABLE_CLAUDE_CODE`        | boolean | Deaktiver læsning fra `.claude` (prompt + færdigheder)                |
| `BOLT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Deaktiver læsning `~/.claude/CLAUDE.md`                               |
| `BOLT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Deaktiver indlæsning af `.claude/skills`                              |
| `BOLT_DISABLE_MODELS_FETCH`       | boolean | Deaktivering af modeller fra eksterne kilder                          |
| `BOLT_FAKE_VCS`                   | string  | Falsk VCS-udbyder til testformål                                      |
| `BOLT_CLIENT`                     | string  | Klient-id (standard til `cli`)                                        |
| `BOLT_ENABLE_EXA`                 | boolean | Aktiver Exa-websøgeværktøjer                                          |
| `BOLT_SERVER_PASSWORD`            | string  | Aktiver grundlæggende godkendelse for `serve`/`web`                   |
| `BOLT_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `opencode`) |
| `BOLT_MODELS_URL`                 | string  | Brugerdefineret URL til hentning af modelkonfiguration                |
...
| Variabel                                        | Skriv   | Beskrivelse                                          |
| ----------------------------------------------- | ------- | ---------------------------------------------------- |
| `BOLT_EXPERIMENTAL`                         | boolean | Aktiver eksperimentelle funktioner under samleflaget |
| `BOLT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Aktiver ikonopdagelse                                |
| `BOLT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Deaktiver kopi ved valg i TUI                        |
| `BOLT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | nummer  | Standard timeout for bash-kommandoer i ms            |
| `BOLT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | nummer  | Maks. output-tokens for LLM-svar                     |
| `BOLT_EXPERIMENTAL_FILEWATCHER`             | boolean | Aktiver filovervågning for hele dir                  |
| `BOLT_EXPERIMENTAL_OXFMT`                   | boolean | Aktiver oxfmt formatter                              |
| `BOLT_EXPERIMENTAL_LSP_TOOL`                | boolean | Aktive eksperimenter LSP værktøj                     |
| `BOLT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Deaktiver filovervågning                             |
| `BOLT_EXPERIMENTAL_EXA`                     | boolean | Aktive eksperimenter Exa-funktioner                  |
| `BOLT_EXPERIMENTAL_LSP_TY`                  | boolean | Aktiver TY LSP for python-filer                      |
| `BOLT_EXPERIMENTAL_PLAN_MODE`               | boolean | Aktiver plantilstand                                 |
| `BOLT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Aktiver baggrundsopgaver for subagenter              |
| `BOLT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Aktiver eksperimentelt hændelsessystem               |
| `BOLT_EXPERIMENTAL_NATIVE_LLM`              | boolean | Aktiver native LLM-anmodningssti                     |
| `BOLT_EXPERIMENTAL_PARALLEL`                | boolean | Aktiver parallel udførelse af websøgning             |
| `BOLT_EXPERIMENTAL_SCOUT`                   | boolean | Aktiver Scout-subagent                               |
| `BOLT_EXPERIMENTAL_WORKSPACES`              | boolean | Aktiver workspace-understøttelse                     |
