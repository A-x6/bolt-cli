---
title: TUI
description: Brug af Bolt-terminalbrugergrænsefladen.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Bolt giver en interaktiv terminalgrænseflade eller TUI til at arbejde med dine projekter med en LLM.

At køre Bolt starter TUI for den nuværende mappe.
...
Når du bruger Bolt TUI, kan du skrive `/` efterfulgt af et kommandonavn for hurtigt at udføre handlinger. For eksempel:
...
Tilføj en udbyder til Bolt. Lader dig vælge fra tilgængelige udbydere og tilføje deres API-nøgler.
...
Dette er adskilt fra `bolt.json`, som konfigurerer server-/kørselstidsadfærd.
...
- `leader_timeout` - Styrer hvor længe Bolt venter efter leader key. Standard er `2000`.
...
Brug `BOLT_TUI_CONFIG` til at indlæse en brugerdefineret TUI-konfigurationssti.
...
- `sound_pack` - Sound pack-ID der skal bruges. Standard er `bolt.default`.
