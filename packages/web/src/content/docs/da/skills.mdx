...
Agentferdigheter lar Bolt oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Bolt søker etter disse stedene:

- Prosjektkonfigurasjon: `.bolt/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Bolt opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.bolt/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.
...
Lag `.bolt/skills/git-release/SKILL.md` slik:
...
Bolt viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `bolt.json`:
...
**For innebygde agenter** (i `bolt.json`):
