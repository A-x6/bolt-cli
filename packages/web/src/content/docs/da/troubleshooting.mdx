...
For at fejlfinde problemer med Bolt, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Bolt Desktop kører en lokal Bolt-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Bolt`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux** (ældre installationer): `~/.local/share/opencode/bolt.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\bolt.jsonc`
...
Bolt kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Bolt kan genopbygge den.

1. Afslut Bolt Desktop helt.
2. Slet cache-mappen:
...
3. Start Bolt Desktop på ny.

---

### Løs problemer med serverforbindelse

Bolt Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `bolt.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `BOLT_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `BOLT_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Bolt Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Bolt.

---

### Meddelelser vises ikke

Bolt Desktop viser kun systemvarsler når:

- varsler er aktiveret for Bolt i dine OS-indstillinger, og
...
1. Afslut Bolt Desktop.
2. Find og slet disse filer (de findes i Bolt Desktop-appens datamappe):

- `bolt.settings.dat` (skrivebordsstandardserver URL)
- `bolt.global.dat` og `bolt.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Bolt:
