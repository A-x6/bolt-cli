---
title: Plugins
description: Skriv dine egne plugins for at udvide Bolt.
---

Plugins giver dig mulighed for at udvide Bolt ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Bolts standardadfærd.
...
- `.bolt/plugins/` - Plugins på projektniveau
...
1. Global konfiguration (`~/.config/opencode/bolt.json`)
2. Projektkonfiguration (`bolt.json`)
3. Global plugin-mappe (`~/.config/opencode/plugins/`)
4. Projekt plugin bibliotek (`.bolt/plugins/`)
...
Bolt kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide bolt.
...
:::note
Hvis du bruger Bolt desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://bolt.ai/docs/sdk) for detaljer.
