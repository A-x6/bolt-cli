---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Bolt.
---

import config from "../../../../config.mjs"
export const console = config.console

Bolt bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Bolt-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i Bolt-konfigurationen.
...
Bolt Zen er en liste over modeller leveret af Bolt-teamet, som er blevet testet og verificeret til at fungere godt med Bolt. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Bolt Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg opencode og gå til [bolt.ai/auth](https://bolt.ai/auth).
...
Det fungerer som alle andre udbydere i Bolt og er helt valgfrit at bruge.

---

## Bolt Go

Bolt Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Bolt-teamet, som er testet og verificeret til at fungere godt med Bolt.

1. Kør kommandoen `/connect` i TUI, vælg `Bolt Go`, og gå til [bolt.ai/auth](https://bolt.ai/zen).
...
Sådan bruger du Amazon Bedrock med Bolt:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `bolt.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Bolt understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Bolt bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Bolt
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`bolt.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Bolt:
...
Sådan bruger du Google Vertex AI med Bolt:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem bolt, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver Bolt-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Bolt-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for Bolt. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Bolt:
...
3. Kopier API-nøglen til brug i Bolt.
...
6. **Vigtigt**: Før du bruger skymodeller i Bolt, skal du hente modelinformationen lokalt:
...
Bolt Zen er en liste over testede og verificerede modeller leveret af Bolt-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Bolt Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Bolt Zen**.

   ```txt
   /connect
   ```

3. Indtast Bolt API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Bolt:
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med bolt. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `bolt.json`-filen i projektkataloget:
...
`limit`-felterne lader Bolt forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
