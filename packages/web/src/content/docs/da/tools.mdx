...
Her er alle de innebygde verktøyene som er tilgængelige i Bolt.
...
:::note
Dette verktøyet er bare tilgængelig når `BOLT_EXPERIMENTAL_LSP_TOOL=true` (eller `BOLT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Bolt-leverandøren eller når miljøvariabelen `BOLT_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
