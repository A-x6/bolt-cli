...
Bolt uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Start Bolt with `--auto` to automatically approve permission requests that are not explicitly denied.

```bash
opencode --auto
```

You can also use auto mode with [`bolt run`](/docs/cli#run).
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Bolt was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
Bolt permissions are keyed by tool name, plus a couple of safety guards:
...
If you don’t specify anything, Bolt starts from permissive defaults:
...
When Bolt prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current Bolt session)
