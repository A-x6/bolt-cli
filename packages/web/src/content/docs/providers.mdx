---
title: Providers
description: Using any LLM provider in Bolt.
---

import config from "../../../config.mjs"
export const console = config.console

Bolt uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Bolt config.
...
You can customize the providers through the `provider` section in your Bolt
config.
...
Bolt Zen is a list of models provided by the Bolt team that have been
tested and verified to work well with Bolt. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Bolt Zen.
:::

1. Run the `/connect` command in the TUI, select `Bolt Zen`, and head to [bolt.ai/auth](https://bolt.ai/zen).
...
It works like any other provider in Bolt and is completely optional to use.

---

## Bolt Go

Bolt Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Bolt team that have been
tested and verified to work well with Bolt.

1. Run the `/connect` command in the TUI, select `Bolt Go`, and head to [bolt.ai/auth](https://bolt.ai/zen).
...
To use Amazon Bedrock with Bolt:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `bolt.json`:

   ```json title="bolt.json"
   {
     "$schema": "https://bolt.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
Bolt. Anthropic explicitly prohibits this.

Previous versions of Bolt came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Bolt with zero setup:
...
```json title="bolt.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://bolt.ai/config.json",
  "provider": {
    "atomic-chat": {
...
Bolt does not query Azure management APIs or discover deployments. Select a model whose catalog name matches your deployment, or configure its deployment name explicitly:
...
Assign your identity the inference role required by the deployment: **Cognitive Services OpenAI User** for Azure OpenAI models or **Cognitive Services User** for other Foundry models. Bolt refreshes access tokens through the Azure CLI, including versions earlier than 2.54.0, so you only need to sign in again when the CLI session expires.
...
Bolt supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Bolt uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Bolt. Sign in and approve.

   :::note
   Bolt requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Bolt surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Bolt forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Bolt is experimental. Features, configuration, and
behavior may change in future releases.
:::

Bolt integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
Bolt uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Bolt
to only use your own GitLab-hosted instance, add the following to your
`bolt.json` file. It is also recommended to disable session sharing.
...
When a `duo-workflow-*` model is selected, Bolt will:
...
4. Route tool execution requests through Bolt's permission-gated tool system
...
To use Google Vertex AI with Bolt:
...
In the event you see a feature or model from Helicone that isn't configured automatically through bolt, you can always configure it yourself.
...
```jsonc title="~/.config/opencode/bolt.jsonc"
{
  "$schema": "https://bolt.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Bolt conversation as a session in Helicone.
...
```json title="bolt.json"
{
  "plugin": ["bolt-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Bolt conversation listed as a separate session.
...
```json title="bolt.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://bolt.ai/config.json",
  "provider": {
    "llama.cpp": {
...
```json title="bolt.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://bolt.ai/config.json",
  "provider": {
    "lmstudio": {
...
:::tip
Ollama can automatically configure itself for Bolt. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::

```json title="bolt.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://bolt.ai/config.json",
  "provider": {
    "ollama": {
...
3. Copy the API key for use in Bolt.
...
6. **Important**: Before using cloud models in Bolt, you must pull the model information locally:
...
Bolt Zen is a list of tested and verified models provided by the Bolt team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Bolt Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Bolt Zen**.

   ```txt
   /connect
   ```

3. Enter your Bolt API key.
...
5. Complete sign-in in the browser that opens. Bolt captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
The model catalog is provided automatically. A minimal `bolt.json` is all that's needed:
...
2. Select **SuperGrok Subscription**. Bolt opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by Bolt. Bolt polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
Bolt refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
:::tip
You can use any OpenAI-compatible provider with bolt. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in bolt.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `bolt.json` file in your project directory:

   ```json title="bolt.json" ""myprovider"" {5-15}
   {
     "$schema": "https://bolt.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="bolt.json" {9,11,17-20}
{
  "$schema": "https://bolt.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Bolt to understand how much context you have left. Standard providers pull these from models.dev automatically.
