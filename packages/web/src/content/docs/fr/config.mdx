---
title: Configuration
description: Utilisation de la configuration JSON Bolt.
---

Vous pouvez configurer Bolt à l'aide d'un fichier de configuration JSON.

---

## Format

Bolt prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
2. **Configuration globale** (`~/.config/opencode/bolt.json`) - préférences utilisateur
3. **Configuration personnalisée** (`BOLT_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`bolt.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.bolt`** - agents, commandes, plugins
6. **Configuration en ligne** (`BOLT_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.bolt` et `~/.config/opencode` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Placez votre configuration globale Bolt dans `~/.config/opencode/bolt.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.
...
Ajoutez `bolt.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque Bolt démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `BOLT_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `BOLT_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.bolt`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.bolt`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`bolt.ai/config.json`**](https://bolt.ai/config.json).

La configuration TUI utilise [**`bolt.ai/tui.json`**](https://bolt.ai/tui.json).
...
Utilisez `BOLT_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `bolt.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Bolt.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `bolt.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Bolt via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Bolt essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/opencode/agents/` ou `.bolt/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Bolt reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`bolt run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/opencode/commands/` ou `.bolt/commands/`. [En savoir plus ici](/docs/commands).
...
Bolt téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
[Plugins](/docs/plugins) étendent Bolt avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.bolt/plugins/` ou `~/.config/opencode/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre Bolt à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
