---
title: Plugins
description: Écrivez vos propres plugins pour étendre Bolt.
---

Les plugins vous permettent d'étendre Bolt en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Bolt.
...
- `.bolt/plugins/` - Plugins au niveau du projet
...
1. Configuration globale (`~/.config/opencode/bolt.json`)
2. Configuration du projet (`bolt.json`)
3. Répertoire global des plugins (`~/.config/opencode/plugins/`)
4. Répertoire des plugins du projet (`.bolt/plugins/`)
...
Bolt exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre bolt.
...
:::note
Si vous utilisez l'application de bureau Bolt, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://bolt.ai/docs/sdk) pour plus de détails.
