...
Pour déboguer les problèmes avec Bolt, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
Bolt Desktop exécute un serveur Bolt local (le side-car `opencode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Bolt` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** : `~/.config/opencode/bolt.jsonc` (ou `~/.config/opencode/bolt.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/opencode/bolt.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\bolt.jsonc`
...
Bolt peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Bolt puisse le reconstruire.

1. Quittez complètement Bolt Desktop.
...
3. Redémarrez le bureau Bolt.

---

### Résoudre les problèmes de connexion au serveur

Bolt Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `bolt.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `BOLT_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `BOLT_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Bolt Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.
...
Bolt Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Bolt dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Bolt.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Bolt Desktop) :

- `bolt.settings.dat` (serveur de bureau par défaut URL)
- `bolt.global.dat` et `bolt.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Bolt :
