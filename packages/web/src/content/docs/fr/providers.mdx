---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Bolt.
---

import config from "../../../../config.mjs"
export const console = config.console

Bolt utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Bolt.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Bolt
configuration.
...
Bolt Zen est une liste de modèles fournis par l'équipe Bolt qui ont été
testé et vérifié pour fonctionner correctement avec Bolt. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Bolt Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez opencode et dirigez-vous vers [bolt.ai/auth](https://bolt.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Bolt et son utilisation est totalement facultative.

---

## Bolt Go

Bolt Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Bolt qui ont été
testé et vérifié pour fonctionner correctement avec Bolt.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Bolt Go` et rendez-vous sur [bolt.ai/auth](https://bolt.ai/zen).
...
Pour utiliser Amazon Bedrock avec Bolt :
...
Pour une configuration spécifique au projet ou persistante, utilisez `bolt.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Bolt n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez également ajouter des modèles via votre configuration bolt.
...
pour stocker le jeton dans le stockage d'authentification bolt.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Bolt utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Bolt
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `bolt.json`. Il est également recommandé de désactiver le partage de session.
...
Pour utiliser Google Vertex AI avec Bolt :
...
Définissez-les lors de l'exécution de bolt.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via bolt, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation Bolt en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Bolt répertoriée comme une session distincte.
...
:::tip
Ollama peut se configurer automatiquement pour Bolt. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Bolt :
...
3. Copiez la clé API à utiliser dans Bolt.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Bolt, vous devez extraire les informations du modèle localement :
...
Bolt Zen est une liste de modèles testés et vérifiés fournie par l'équipe Bolt. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Bolt Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Bolt Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Bolt API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration bolt.
...
5. Vous pouvez également les personnaliser via votre configuration bolt. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration bolt. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec bolt. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `bolt.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à Bolt de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration opencode et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration bolt.
