---
title: Règles
description: Définissez des instructions personnalisées pour bolt.
---
...
Pour créer un nouveau fichier `AGENTS.md`, vous pouvez exécuter la commande `/init` dans bolt.
...
Vous pouvez également avoir des règles globales dans un fichier `~/.config/opencode/AGENTS.md`. Ceci est appliqué à toutes les sessions bolt.
...
Pour les utilisateurs migrant depuis Claude Code, Bolt prend en charge les conventions de fichiers de Claude Code comme solutions de secours :
...
Vous pouvez spécifier des fichiers d'instructions personnalisés dans votre `bolt.json` ou dans le `~/.config/opencode/bolt.json` global. Cela vous permet, à vous et à votre équipe, de réutiliser les règles existantes plutôt que d'avoir à les dupliquer dans AGENTS.md.
...
### Utilisation de bolt.json

L'approche recommandée consiste à utiliser le champ `instructions` dans `bolt.json` :
...
:::tip
Pour les monorepos ou les projets avec des normes partagées, l'utilisation de `bolt.json` avec des modèles globaux (comme `packages/*/AGENTS.md`) est plus maintenable que les instructions manuelles.
:::
