...
Les compétences d'agent permettent à Bolt de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Bolt recherche ces emplacements :

- Configuration du projet : `.bolt/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Bolt remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.bolt/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.
...
Créez `.bolt/skills/git-release/SKILL.md` comme ceci :
...
Bolt répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `bolt.json` :
...
**Pour les agents intégrés** (dans `bolt.json`) :
