...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Bolt.
...
Voici plusieurs modèles qui fonctionnent bien avec Bolt, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Bolt.

```json title="bolt.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Bolt Zen](/docs/zen), vous utiliserez `opencode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="bolt.jsonc" {7-12,19-24}
{
  "$schema": "https://bolt.ai/config.json",
  "provider": {
    "openai": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Bolt est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Bolt est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
Lorsque Bolt démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Bolt.
