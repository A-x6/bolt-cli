...
Da biste otklonili probleme s Bolt, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Bolt pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
Bolt Desktop pokreće lokalni Bolt server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Bolt` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/opencode/bolt.jsonc` (ili `~/.config/opencode/bolt.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/opencode/bolt.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencode\bolt.jsonc`
...
Bolt također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Bolt mogao ponovo izgraditi.

1. Potpuno zatvorite Bolt Desktop.
...
3. Ponovo pokrenite Bolt Desktop.

---

### Rješavanje problema sa vezom na serveru

Bolt Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `bolt.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `BOLT_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `BOLT_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Bolt Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Bolt karakteristikama.

---

### Obavještenja se ne prikazuju

Bolt Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Bolt u postavkama vašeg OS-a, i
...
1. Zatvorite Bolt Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Bolt Desktop aplikacije):

- `bolt.settings.dat` (URL zadanog servera za desktop)
- `bolt.global.dat` i `bolt.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
2. **Pridružite se našem Discordu**
   ...
   [**bolt.ai/discord**](https://bolt.ai/discord)
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Bolt dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite Bolt da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Bolt da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Bolt će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
