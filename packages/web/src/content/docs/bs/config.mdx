---
title: Konfiguracija
description: Korištenje Bolt JSON konfiguracije.
---

Možete konfigurirati Bolt koristeći JSON konfiguracijski fajl.

---

## Format

Bolt podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
2. **Globalna konfiguracija** (`~/.config/opencode/bolt.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`BOLT_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`bolt.json` u projektu) - postavke specifične za projekat
5. **`.bolt` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`BOLT_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.bolt` i `~/.config/opencode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Postavite svoju globalnu Bolt konfiguraciju u `~/.config/opencode/bolt.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Dodajte `bolt.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Bolt pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `BOLT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `BOLT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.bolt` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.bolt`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`bolt.ai/config.json`**](https://bolt.ai/config.json).

TUI konfiguracija koristi [**`bolt.ai/tui.json`**](https://bolt.ai/tui.json).
...
Koristite `BOLT_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `bolt.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Bolt server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `bolt.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Bolt konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Bolt pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/opencode/agents/` ili `.bolt/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Bolt će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`bolt run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/opencode/commands/` ili `.bolt/commands/`. [Saznajte više ovdje](/docs/commands).
...
Bolt će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Bolt **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Bolt sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.bolt/plugins/` ili `~/.config/opencode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Bolt da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
