---
title: Pravila
description: Postavite prilagodena uputstva za bolt.
---
...
Da kreirate novu `AGENTS.md` datoteku, pokrenite `/init` komandu u bolt.
...
Za korisnike koji prelaze sa Claude Code, Bolt podrzava i Claude konvencije datoteka kao rezervu:
...
Mozete navesti prilagodene datoteke uputstava u `bolt.json` ili globalnom `~/.config/opencode/bolt.json`. Tako vi i tim ponovo koristite postojeca pravila bez dupliranja u AGENTS.md.
...
### Korištenje bolt.json

Preporuceni pristup je da koristite `instructions` polje u `bolt.json`:
...
:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `bolt.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::
