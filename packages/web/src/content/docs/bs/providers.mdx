---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Bolt.
---

import config from "../../../../config.mjs"
export const console = config.console

Bolt koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Bolt konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Bolt
config.
...
Bolt Zen je lista modela koje je obezbedio Bolt tim koji su bili
testirano i potvrđeno da dobro radi sa Bolt. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Bolt Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite opencode i idite na [bolt.ai/auth](https://bolt.ai/auth).
...
Radi kao i svaki drugi provajder u Bolt i potpuno je opcionalan za korištenje.

---

## Bolt Go

Bolt Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Bolt tim i koji su testirani i verificirani da dobro rade s Bolt-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Bolt Go` i idite na [bolt.ai/auth](https://bolt.ai/zen).
...
Da biste koristili Amazon Bedrock s Bolt:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `bolt.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Bolt.
:::
...
Bolt koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Bolt
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`bolt.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Za korištenje Google Vertex AI s Bolt:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće bolt.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Bolt razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Bolt razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za Bolt. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Bolt:
...
3. Kopirajte API ključ za korištenje u Bolt.
...
6. **Važno**: Prije upotrebe modela oblaka u Bolt, morate lokalno povući informacije o modelu:
...
Bolt Zen je lista testiranih i verifikovanih modela koju je obezbedio Bolt tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Bolt Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Bolt Zen**.

```txt
   /connect
```

3. Unesite svoj Bolt API ključ.
...
4. Kreirajte ili ažurirajte svoju `bolt.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Bolt da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite Bolt konfiguraciju i:
