...
Agent skills omogucavaju Bolt da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Bolt pretrazuje ove lokacije:

- Konfiguracija projekta: `.bolt/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Bolt ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.bolt/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.
...
Kreirajte `.bolt/skills/git-release/SKILL.md` ovako:
...
Bolt navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `bolt.json`:
...
**Za ugradene agente** (u `bolt.json`):
