...
Možete dodati vanjske alate u Bolt koristeći _Model Context Protocol_, ili MCP. Bolt podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Bolt Config](https://bolt.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Bolt automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Bolt će:
...
Ako server zahtijeva autentifikaciju, Bolt će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Bolt sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Bolt, zajedno s ugrađenim alatima. Možete njima upravljati kroz Bolt konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Bolt sa vašim Sentry nalogom.
