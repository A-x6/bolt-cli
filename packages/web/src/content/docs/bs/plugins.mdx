---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Bolt.
---

Dodaci vam omogućavaju da proširite Bolt spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Bolt.
...
- `.bolt/plugins/` - Dodaci na nivou projekta
...
1. Globalna konfiguracija (`~/.config/opencode/bolt.json`)
2. Konfiguracija projekta (`bolt.json`)
3. Globalni direktorij dodataka (`~/.config/opencode/plugins/`)
4. Direktorij dodataka projekta (`.bolt/plugins/`)
...
Bolt pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Bolt.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://bolt.ai/docs/sdk) za detalje.
