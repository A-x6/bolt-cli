...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Bolt dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Bolt.
...
:::note
Ovaj alat je dostupan samo kada je `BOLT_EXPERIMENTAL_LSP_TOOL=true` (ili `BOLT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Bolt provajdera ili kada je varijabla `BOLT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Bolt:
