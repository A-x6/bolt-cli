---
title: TUI
description: Korzystanie z interfejsu użytkownika terminala bolt.
---
...
Korzystając z TUI bolt, możesz wpisać `/`, po którym następuje nazwa polecenia, aby szybko wykonać działania. Na przykład:
...
Dodaj dostawcę do bolt. Umożliwia wybór dostępnych dostawców i dodanie ich kluczy API.
...
Jest to oddzielny plik od `bolt.json`, który konfiguruje zachowanie serwera/runtime.
...
- `leader_timeout` - Określa, jak długo Bolt czeka po naciśnięciu leader key. Domyślnie `2000`.
...
- `sound_pack` - ID sound pack do użycia. Domyślnie `bolt.default`.
