---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w bolt.
---
...
2. Skonfiguruj dostawcę w konfiguracji bolt.
...
Bolt Zen to lista modeli dostarczonych przez zespół bolt, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z bolt. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Bolt Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Bolt Zen` i przejdź do [bolt.ai/auth](https://bolt.ai/zen).
...
Bolt Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół bolt, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z bolt.

1. Uruchom polecenie `/connect` w TUI, wybierz `Bolt Go` i przejdź do [bolt.ai/auth](https://bolt.ai/zen).
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `bolt.json`:
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji bolt.
...
`bolt.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania bolt.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą bolt, zawsze możesz ją skonfigurować samodzielnie.
...
:::tip
Ollama może automatycznie skonfigurować się pod kątem bolt. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opencode).
:::
...
3. Skopiuj klucz API do użycia w bolt.
...
Bolt Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół bolt. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Bolt Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Bolt Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API bolt.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji bolt.
...
5. Możesz je także dostosować za pomocą konfiguracji bolt. Oto przykład określenia dostawcy
...
Możesz także dostosować modele za pomocą konfiguracji bolt. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z bolt. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `bolt.json` w katalogu projektu:
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację opencode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji bolt.
