...
Narzędzia specjalistyczne to funkcje, które tworzysz i które LLM może wywoływać podczas rozmów. Współpracują one z [wbudowanymi narzędziami](/docs/tools) bolt, takimi jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.bolt/tools/` swojego projektu.
