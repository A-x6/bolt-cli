---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć bolt.
---

Wtyczki umożliwiające wyłączenie opencode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania bolt.
...
- `.bolt/plugins/` - Wtyczki na poziomie projektu
...
1. Konfiguracja globalna (`~/.config/opencode/bolt.json`)
2. Konfiguracja projektu (`bolt.json`)
3. Globalny katalog wtyczek (`~/.config/opencode/plugins/`)
4. Katalog wtyczek projektu (`.bolt/plugins/`)
...
Oto kilka dostępnych wtyczek, które można udostępnić do bolt.
...
:::note
Jeśli korzystasz z aplikacji komputerowej bolt, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod bolt. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://bolt.ai/docs/sdk).
