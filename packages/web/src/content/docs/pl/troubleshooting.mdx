...
Aby debugować problemy z bolt, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- Tylko macOS: menu `Bolt` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/opencode/bolt.jsonc` (lub `~/.config/opencode/bolt.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opencode/bolt.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\bolt.jsonc`
...
Jeśli Twój `bolt.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `BOLT_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `BOLT_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami bolt.
...
- `bolt.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `bolt.global.dat` i `bolt.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
2. Uruchom ponownie kod bolt, aby ponownie zainstalować najnowsze pakiety dostawców
