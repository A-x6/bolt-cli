...
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.bolt/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.
...
Utwórz `.bolt/skills/git-release/SKILL.md` w ten sposób:
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `bolt.json`:
...
**Dla agentów wbudowanych** (w `bolt.json`):
