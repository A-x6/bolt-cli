...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Bolt posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Bolt.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `BOLT_EXPERIMENTAL_LSP_TOOL=true` (lub `BOLT_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Bolt lub gdy zmienna środowiskowa `BOLT_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Bolt:
