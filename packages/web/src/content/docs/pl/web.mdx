...
:::caution
Jeśli `BOLT_SERVER_PASSWORD` nie jest ustawione, serwer będzie niezabezpieczony. Jest to dobre rozwiązanie do użytku lokalnego, ale powinno być ustawione na dostęp do sieci.
:::
...
To automatycznie ustawia nazwę hosta na `0.0.0.0` i anonsuje serwer jako `bolt.local`.
...
Aby chronić dostęp, ustaw hasło za pomocą zmiennej środowiskowej `BOLT_SERVER_PASSWORD`:

```bash
BOLT_SERVER_PASSWORD=secret opencode web
```

Domyślna nazwa użytkownika to `opencode`, ale można ją zmienić za pomocą `BOLT_SERVER_USERNAME`.

---

## Korzystanie z interfejsu internetowego

Po uruchomieniu interfejs sieciowy zapewnia dostęp do sesji bolt.
...
Możesz także skonfigurować ustawienia serwera w pliku konfiguracyjnym `bolt.json`:
