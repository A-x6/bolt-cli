---
title: Serwery LSP
description: Bolt integruje się z serwerami LSP.
---

Bolt może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

Bolt posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `BOLT_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i Bolt otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji Bolt.
