...
2. **Dodaj przepływ pracy**
   ...
   Dodaj zdalny plik pracy do `.github/workflows/bolt.yml` w swoim repozytorium. wystąpił, że ustawiłeś sędziego `model` i wymagany klucz API w `env`.
...
- `model`: Model używany z bolt. Przyjmuje format `provider/model`. To **wymagane**.
...
- `share`: Czyć funkcję sesji bolt. Domyślnie **true** dla repozytoriów publicznych.
- `prompt`: Opcjonalny niestandardowy monit o zastąpienie przestrzegania zachowania. Wykorzystanie tego, aby dostosować sposób przetwarzania przez bolt.
...
| `pull_request`                | PR otwarty lub zaktualizowany                    | Automatycznie wyzwalaj bolt, gdy PR są otwierane, synchronizowane lub ponownie otwierane. Przydatne w przypadku automatycznych znajomych.         |
