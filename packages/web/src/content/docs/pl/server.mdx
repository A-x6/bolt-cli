...
Komenda `opencode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient bolt.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `bolt.local` |
...
Ustaw `BOLT_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `opencode` lub ustaw `BOLT_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `opencode serve`, jak i `opencode web`.
...
:::tip
Użyj serwera opencode do programowej interakcji z bolt.
:::
