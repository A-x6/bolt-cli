---
title: Config
description: Using the Bolt JSON config.
---

You can configure Bolt using a JSON config file.

---

## Format

Bolt supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="bolt.jsonc"
{
...
2. **Global config** (`~/.config/opencode/bolt.json`) - user preferences
3. **Custom config** (`BOLT_CONFIG` env var) - custom overrides
4. **Project config** (`bolt.json` in project) - project-specific settings
5. **`.bolt` directories** - agents, commands, plugins
6. **Inline config** (`BOLT_CONFIG_CONTENT` env var) - runtime overrides
...
:::note
The `.bolt` and `~/.config/opencode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global Bolt config in `~/.config/opencode/bolt.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
Add `bolt.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Bolt starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `BOLT_CONFIG` environment variable.

```bash
export BOLT_CONFIG=/path/to/my/custom-config.json
...
Specify a custom config directory using the `BOLT_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.bolt` directory, and should
follow the same structure.

```bash
export BOLT_CONFIG_DIR=/path/to/my/config-directory
bolt run "Hello world"
```

The custom directory is loaded after the global config and `.bolt` directories, so it **can override** their settings.
...
Drop an `bolt.json` or `bolt.jsonc` file in the system managed config directory:
...
On macOS, Bolt reads managed preferences from the `ai.bolt.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Bolt checks these paths:

1. `/Library/Managed Preferences/<user>/ai.bolt.managed.plist`
2. `/Library/Managed Preferences/ai.bolt.managed.plist`

The plist keys map directly to `bolt.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.bolt.managed` PayloadType. The Bolt config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.bolt.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.bolt.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.bolt</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`bolt.ai/config.json`**](https://bolt.ai/config.json).

TUI config uses [**`bolt.ai/tui.json`**](https://bolt.ai/tui.json).
...
Use `BOLT_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `bolt.json` are deprecated and automatically migrated when possible.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Bolt server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `bolt.local`. Useful for running multiple instances on the same network.
...
If not specified, Bolt will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Bolt config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Bolt tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Bolt actions on configured resources. Currently, policies can control which providers Bolt may use.
...
Bolt normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Bolt omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/opencode/agents/` or `.bolt/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Bolt will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`bolt run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/opencode/commands/` or `.bolt/commands/`. [Learn more here](/docs/commands).
...
Bolt uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Bolt will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
[Plugins](/docs/plugins) extend Bolt with custom tools, hooks, and integrations.

Place plugin files in `.bolt/plugins/` or `~/.config/opencode/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="bolt.json"
{
  "$schema": "https://bolt.ai/config.json",
  "plugin": ["bolt-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Bolt to only use specific providers rather than disabling them one by one.
