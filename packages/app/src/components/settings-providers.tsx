...
const PROVIDER_NOTES = [
  { match: (id: string) => id === "bolt", key: "dialog.provider.bolt.note" },
  { match: (id: string) => id === "bolt-go", key: "dialog.provider.boltGo.tagline" },
...
const SettingsProvidersContent: Component<{ onBack?: () => void }> = (props) => {
  ...
  const connected = createMemo(() => {
    return providers
      .connected()
      .filter((p) => p.id !== "bolt" || Object.values(p.models).find((m) => m.cost?.input))
  })
                ...
                <div class="flex flex-wrap items-center justify-between gap-4 min-h-16 py-3 border-b border-border-weak-base last:border-none">
                  <div class="flex flex-col min-w-0">
                    <div class="flex items-center gap-x-3">
                      ...
                      <Show when={item.id === "bolt"}>
                        <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
                      </Show>
                      <Show when={item.id === "bolt-go"}>
                        <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
                      </Show>
                    </div>
