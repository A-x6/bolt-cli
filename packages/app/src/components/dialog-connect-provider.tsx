import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@bolt-ai/client/promise"
import { Button } from "@bolt-ai/ui/button"
import { useDialog } from "@bolt-ai/ui/context/dialog"
import { Dialog } from "@bolt-ai/ui/dialog"
import { Icon } from "@bolt-ai/ui/icon"
import { IconButton } from "@bolt-ai/ui/icon-button"
import { List, type ListRef } from "@bolt-ai/ui/list"
import { ProviderIcon } from "@bolt-ai/ui/provider-icon"
import { Spinner } from "@bolt-ai/ui/spinner"
import { Tag } from "@bolt-ai/ui/tag"
import { TextField } from "@bolt-ai/ui/text-field"
import { ButtonV2 } from "@bolt-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@bolt-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@bolt-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "bolt-go") return language.t("dialog.provider.boltGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "bolt"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.bolt.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["bolt", "bolt-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "bolt" || provider.id === "bolt-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "bolt"
                                ? "dialog.provider.bolt.tagline"
                                : "dialog.provider.boltGo.tagline",
                            )}
                          </span>
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          if (props.provider === "bolt" && platform.platform === "desktop") {
            const url = new URL(x.data.url)
            url.searchParams.set("client_id", "bolt-desktop")
            x.data.url = url.href
          }
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "bolt"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.boltZen.line1")}</div>
              <div>{language.t("provider.connect.boltZen.line2")}</div>
              <div>
                {language.t("provider.connect.boltZen.visit.prefix")}
                <ExternalLink
                  href="https://bolt.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.boltZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.boltZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "bolt"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.boltZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.boltZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.boltZen.visit.prefix")}
                <ExternalLink href="https://bolt.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.boltZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.boltZen.visit.suffix")}
              </div>
