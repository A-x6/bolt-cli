...
import { type Session } from "@bolt-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  compareSessionTime,
...
describe("layout deep links", () => {
  ...
  test("ignores non-project deep links", () => {
    expect(parseDeepLink("bolt://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("bolt://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("bolt://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("bolt://open-project")).toBeUndefined()
    expect(parseDeepLink("bolt://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "bolt://open-project?directory=/a",
      "bolt://other?directory=/b",
      "bolt://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("bolt://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("bolt://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("bolt://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("bolt://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "bolt://new-session?directory=/a",
      "bolt://open-project?directory=/b",
      "bolt://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __BOLT__: {
        deepLinks: ["bolt://open-project?directory=/a"],
      },
    } as unknown as Window & { __BOLT__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["bolt://open-project?directory=/a"])
