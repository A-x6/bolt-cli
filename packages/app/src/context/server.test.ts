  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://bolt.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://bolt.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://bolt.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          ...
          lastProject: { local: "/local", "https://bolt.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://bolt.example.com"),
      ),
    ).toEqual({
