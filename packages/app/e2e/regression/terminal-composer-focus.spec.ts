...
const directory = "C:/Bolt/TerminalComposerFocus"
...
test.beforeEach(async ({ page }) => {
  await mockBoltServer(page, {
    ...
    provider: {
      ...
      connected: ["bolt"],
      default: { providerID: "bolt", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
...
function seedCachedTerminal(page: Page) {
  return page.addInitScript(
    ({ terminalKey, ptyID }) => {
      localStorage.setItem("bolt.global.dat:layout", JSON.stringify({ terminal: { height: 320, opened: true } }))
