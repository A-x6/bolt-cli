...
import { Event } from "@bolt-ai/schema/event"
import { SessionStatusEvent } from "@bolt-ai/schema/session-status-event"
import { SessionV1 } from "@bolt-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Bolt/TimelineStability"
...
export const model = { providerID: "bolt", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("bolt.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["bolt"],
    default: { providerID: "bolt", modelID: "claude-opus-4-6" },
  }
}
